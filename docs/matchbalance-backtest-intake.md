...
4. Inspect structure only when needed. MatchBalance converts the captured fixtures into an exact slot graph, including cross-pool games and winner, loser, and pool-rank advancement references. It preserves division capacities, pool capacities, pool names, and fixture order automatically. When every fixture has a match number, those numbers establish replay order; when any real fixture is unnumbered, the saved page order is preserved so an unnumbered pool game cannot be moved behind a playoff. The operator does not approve every division. Final standings are never treated as original seeds. Open **Tournament tiebreak rule** once for the event, confirm the published order and scoring policy from the captured source link, add a short verification note, and save. The current replay supports points, goal differential, goals scored, and wins in any verified order beginning with points, using standard 3/1/0 points and uncapped goal differential. A different criterion, points system, bonus, or goal-differential cap remains a visible software blocker so the run never invents which team advances.
...
8. Open **Backtest**. One readiness table separates your capture and identity decisions from MatchBalance schedule, model, merge-map, and historical-rating checks. The newest eligible local model is selected automatically; `MATCHBALANCE_POINT_IN_TIME_MODEL_ARTIFACT` remains an override. Run one selected cohort or every ready cohort.
...
11. Each run also predicts the unchanged original fixture arrangement. If that internal calibration misses the actual average margin by more than 1.0 goal, misses the actual 4+ blowout rate by more than 10 percentage points, or covers less than 95% of the scored-fixture count, the run remains saved but the sales comparison is withheld.
12. The **Tournament-wide Backtest result** combines the newest compatible result for every cohort. A tournament-director package becomes available only after every cohort finishes and the unchanged-fixture calibration passes.
