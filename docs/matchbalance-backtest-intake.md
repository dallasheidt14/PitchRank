...
4. Inspect structure only when needed. MatchBalance converts the captured fixtures into an exact slot graph, including cross-pool games and winner, loser, and pool-rank advancement references. It preserves division capacities, pool capacities, pool names, and fixture order automatically. The operator does not approve every division. Final standings are never treated as original seeds. Open **Tournament tiebreak rule** once for the event, confirm the published order from the captured source link, add a short verification note, and save. The current replay supports points, goal differential, goals scored, and wins in any verified order beginning with points. A different published criterion remains a visible software blocker so the run never invents which team advances.
