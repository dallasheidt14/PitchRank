...
The optimizer objective and the reported seeding comparison use the same intra-pool matchup set. The output verifies that the projected pair-cost total equals the optimizer's recorded total before reporting a delta. Playoff simulation remains a separate schedule projection and does not silently enter the optimization claim. The optimizer still reports its existing projected 3+ and 5+ goal thresholds; the intake's actual-results baseline continues to define a blowout as 4 or more goals.
