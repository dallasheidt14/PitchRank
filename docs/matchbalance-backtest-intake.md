...
Each modeled arrangement currently reports 95% normal-approximation intervals. Probability intervals use each matchup's predicted Bernoulli variance. The goal-margin interval describes variation across scheduled matchup projections; it is not model or team-strength uncertainty. Full correlated tournament simulation remains required before these intervals can be presented as the complete range of likely tournament outcomes.
