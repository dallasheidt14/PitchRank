...
8. Open **Backtest**. One readiness table separates your capture and identity decisions from MatchBalance schedule, model, merge-map, and historical-rating checks. Save any tiebreak edit before running so the displayed rule and replay rule are identical. The newest eligible local model fitted with the required `poisson_draw_gate` strategy is selected automatically; `MATCHBALANCE_POINT_IN_TIME_MODEL_ARTIFACT` remains an override, but an artifact fitted for another strategy is rejected rather than reinterpreted. Run one selected cohort or every ready cohort.
