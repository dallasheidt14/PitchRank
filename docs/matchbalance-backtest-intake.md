...
4. Inspect structure only when needed. MatchBalance converts the captured fixtures into an exact slot graph, including cross-pool games and winner, loser, and pool-rank advancement references. It preserves division capacities, pool capacities, pool names, and fixture order automatically. The operator does not approve every division. Final standings are never treated as original seeds. If a simulated pool-rank qualifier is tied and the capture contains only a rules link rather than a verified tiebreak order, the run stops instead of inventing an order.
...
Repeat scrapes retain existing source notes, rules links, and cohort corrections. Cohort corrections remain valid only for the same structure. Saves merge operator decisions against the loaded version under the event lock; conflicting edits require reopening the saved intake.
