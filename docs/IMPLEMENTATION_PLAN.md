...
- In `_match_team()`, add age validation: parse age from team_name using `parse_age_token()` (Phase 1.4) and compare against the `age_group` parameter. If they conflict, log a warning for review rather than letting either side silently win. Note the `age_group` parameter here is provider metadata off `game_data`, not `teams.age_group` — the master column is only reached later, when querying candidates — so neither the name nor the parameter is authoritative on its own. Derive the expected cohort rather than hardcoding one: `B14` is birth year 2014, which is U13 in 2026-27 and shifts every Aug 1, so a "conflict" against a hardcoded pair is usually the hardcoding.
