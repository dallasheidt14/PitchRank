...
- **Noted**: 2026-09-16
- **Refs**: branch `fix/self-serve-cancel-canceling`. Shipped differently from the entry: `handleCheckoutCompleted` was a fifth writer and reads the helper too; the Beehiiv `canceling` lifecycle is sent only for `active` subscriptions (trials and past_due get the stored flag but not the sequence); `computeTrialProjection` also excludes canceled trials; the reconcile job writes the helper's value when it fixes another mismatch but deliberately does not compare the flag, because the webhook sends the canceling and reactivation emails only when it sees the stored flag change and a repair there would swallow a late webhook's transition (PR review); the Streamlit sync is split out as IMP-246. Cancellations already scheduled through `cancel_at` before deploy get the stored flag from a one-off records-only write with no Beehiiv sync, so they do not receive the Cancellation Save sequence late, as the owner chose.
