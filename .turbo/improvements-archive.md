...
- **Noted**: 2026-09-16
- **Refs**: branch `fix/self-serve-cancel-canceling`. Shipped differently from the entry: `handleCheckoutCompleted` was a fifth writer and reads the helper too; the Beehiiv `canceling` lifecycle is sent only for `active` subscriptions (trials and past_due get the stored flag but not the sequence); `computeTrialProjection` also excludes canceled trials; the reconcile job compares the flag as well as writing it; the Streamlit sync is split out as IMP-246. The reconcile repair writes the flag without any Beehiiv sync, so cancellations already scheduled through `cancel_at` before deploy get the flag but not the Cancellation Save sequence, as the owner chose.
