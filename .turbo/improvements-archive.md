...
- **Noted**: 2026-09-03
- **Refs**: `20260908120000_lock_down_scrape_requests_writes.sql` on branch `lock-down-scrape-requests-writes`. Took the deny-all shape but kept SELECT: the browser's missing-game toast reads this table with the anon key, and queue contents are not sensitive, so only the write grant and the INSERT policy are removed. Also added the `priority BETWEEN 1 AND 5` CHECK the column only ever had in a COMMENT, and revoked EXECUTE on `claim_queue_items` — it is SECURITY INVOKER, updates this table, and its default ACL left EXECUTE with PUBLIC, which the entry's grant-line reading would have missed. Guarded by `tests/unit/test_scrape_requests_rls_migration.py`, whose browser-reachable test treats a missing `TO` clause as public because all three removed policies were written that way.
