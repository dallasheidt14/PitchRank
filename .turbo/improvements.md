...
### The trajectory chart plots a scoreless period as an honest-looking 0.00 goal differential

- **ID**: IMP-201
- **Status**: open
- **Type**: plan
- **Category**: correctness
- **Where**: `frontend/lib/api.ts` `getTeamTrajectory` (the games read, ~line 592) and `calculatePeriodMetrics` (~line 1427); rendered by `frontend/components/TeamTrajectoryChart.tsx`
- **Why**: `getTeamTrajectory` reads games with only `is_excluded = false` — it filters neither null scores nor future dates, unlike its sibling `getTeamGames`, which does both (`.lte('game_date', today)` at `api.ts:713`). `calculatePeriodMetrics` then skips any game missing a score, so a period made entirely of unscored rows yields `gamesPlayed = 0`, `avg_goals_for = 0`, `avg_goals_against = 0` — and the chart plots `0 - 0 = 0.00`, indistinguishable from a genuine period of exactly even results. Scheduled fixtures that have not been played extend the line past today the same way. Both failure modes read to a customer as "we drew everything", not "we have no data", and they also feed `classifyTrajectoryTrend`, where a run of fake zeros can flip a real improving trend to declining. Fix candidates: filter unscored and future rows out of the read, or carry `games_played` through to the chart and break the line (`connectNulls={false}` is already set on the trend line) rather than drawing a zero. Prefer the second — dropping the rows silently shortens the axis, which is the same illegibility from the other side.
- **Noted**: 2026-09-09
