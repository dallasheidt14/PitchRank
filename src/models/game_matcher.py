...
class GameHistoryMatcher:
    ...
    def _match_team(
        ...
        if team_name and age_group and gender:
            ...
            if fuzzy_match:
                ...
                # Low confidence - still add to review queue with suggestion
                    ...
                    return {"matched": False, "team_id": None, "method": "fuzzy_review_low", "confidence": confidence}
        ...
        if team_name and age_group and gender:
            ...
            logger.debug(
                f"Cannot fuzzy match: missing required fields "
    ...
    def _match_by_alias(
        ...
        try:
            ...
            if result.data:
                ...
                return {"team_id_master": alias["team_id_master"], "confidence": alias.get("match_confidence", 0.9)}
        except Exception as e:
    ...
    def _calculate_match_score(self, provider_team: Dict, candidate: Dict) -> float:
        ...
        if provider_club and candidate_club:
            if HAVE_CLUB_NORMALIZER:
                ...
                if HAVE_RAPIDFUZZ:
                    club_similarity = (
                        rapidfuzz_fuzz.token_sort_ratio(provider_club.lower(), candidate_club.lower()) / 100.0
                    )
                else:
                    club_similarity = SequenceMatcher(None, provider_club.lower(), candidate_club.lower()).ratio()
    ...
    def _get_provider_id(self, provider_code: Optional[str]) -> str:
        ...
        for attempt in range(max_retries):
            try:
                result = self.db.table("providers").select("id").eq("code", provider_code).single().execute()
