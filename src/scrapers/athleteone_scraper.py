...
class AthleteOneScraper:
    ...
    def scrape_conference_games(
        ...
        if not parsed_games:
            self.logger.warning("No games found in conference schedule")
            return []
        ...
        for parsed_game in parsed_games:
            ...
            if not home_team_id:
                home_team_id = self._generate_team_id(parsed_game.home_team_name, parsed_game.match_id, prefix="home")

            away_team_id = parsed_game.away_team_id
            if not away_team_id:
                away_team_id = self._generate_team_id(parsed_game.away_team_name, parsed_game.match_id, prefix="away")
        ...
        self.logger.info(f"Converted {len(parsed_games)} parsed games into {len(game_data_list)} GameData entries")
