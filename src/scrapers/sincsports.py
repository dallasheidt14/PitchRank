...
class SincSportsScraper(BaseScraper):
    ...
    def __init__(self, supabase_client, provider_code: str = "sincsports"):
        ...
        logger.info("Initialized SincSportsScraper")
    ...
    def extract(self, context, days_back: Optional[int] = None) -> List[Dict]:
        ...
        for team in teams:
            try:
                ...
                games = self.scrape_team_games(team["provider_team_id"], days_back=days_back)
                ...
                self.errors.append({"team_id": team["provider_team_id"], "error": str(e)})
    ...
    def _init_http_session(self) -> requests.Session:
        ...
        adapter = HTTPAdapter(
            ...
            pool_maxsize=10,
            max_retries=Retry(
                total=3, backoff_factor=0.5, status_forcelist=[500, 502, 503, 504], allowed_methods=["GET", "HEAD"]
            ),
        )
    ...
    def scrape_team_games(
        self, team_id: str, since_date: Optional[datetime] = None, days_back: Optional[int] = None
    ) -> List[GameData]:
        """
    ...
    def _extract_team_info(self, soup: BeautifulSoup, team_id: str) -> Dict:
        ...
        team_info = {"team_name": None, "club_name": None, "age_group": None, "gender": None}
    ...
    def _parse_games_from_html(
        self, soup: BeautifulSoup, team_id: str, team_info: Dict, since_date: date
    ) -> List[GameData]:
            ...
            for row in game_rows:
                ...
                # If we have multiple of 4 columns (e.g., 16 = 4 games), split into individual games
                    ...
                    for i in range(num_games):
                        ...
                        game = self._parse_game_from_cols(
                            game_cols, team_id, team_info, event_name, division_name, since_date
                        )
                        if game:
                            # Create unique key to avoid duplicates
                            game_key = (game.game_date, game.opponent_id)
