...
class BaseScraper(BaseProvider, ETLPipeline):
    ...
    def extract(self, context) -> List[Dict]:
        ...
        for team in teams:
            try:
                ...
                games = self.scrape_team_games(team["provider_team_id"], since_date=last_scrape)
    ...
    def _get_teams_to_scrape(self) -> List[Dict]:
        ...
        try:
            ...
            while True:
                ...
                result = (
                    self.db.rpc("get_teams_to_scrape", {"p_provider_id": provider_id})
            ...
            while True:
                result = (
                    self.db.table("teams")
                    .select("*")
                    .eq("provider_id", self._get_provider_id())
                    .or_("last_scraped_at.is.null,last_scraped_at.lt." + cutoff_date)
                    .range(offset, offset + page_size - 1)
                    .execute()
                )
    ...
    def _log_team_scrape(self, team_id: str, games_found: int):
        ...
        self.db.table("teams").update({"last_scraped_at": now.isoformat()}).eq("team_id_master", team_id).execute()
