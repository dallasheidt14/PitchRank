...
# The paths this change is about: both decide a state from a club.
STATE_PATHS = ("scripts/assign_team_states.py", "src/models/game_matcher.py")
...
def test_the_state_paths_import_the_shared_module():
    for rel in STATE_PATHS:
        ...
        assert "src.utils.placeholder_clubs" in imported, (
            f"{rel} decides a state from a club and must read the shared placeholder set"
        )
