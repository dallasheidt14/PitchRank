...
class TestSOSValueRanges:
    ...
    def test_powerscore_uses_sos_norm(self, sample_games_df):
        ...
        if len(teams_df) > 0:
            ...
            # Formula: (OFF_WEIGHT*off_norm + DEF_WEIGHT*def_norm + SOS_WEIGHT*sos_norm + PERF_BLEND_WEIGHT*perf_centered) / MAX_THEORETICAL
            # MAX_THEORETICAL = 1.0 + 0.5 * PERF_BLEND_WEIGHT (accounts for perf additive range)
            max_theoretical = 1.0 + 0.5 * cfg.PERF_BLEND_WEIGHT
            expected_ps = (
