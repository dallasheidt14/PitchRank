...
export function useScrapeRequestNotifications() {
  ...
  useEffect(() => {
    // Wrap in try/catch: in React 19, useEffect errors propagate to error boundaries
    // which would crash the entire GameHistoryTable section
    ...
    try {
        ...
        .on(
          ...
          (payload) => {
            try {
              ...
              console.error('[useScrapeRequestNotifications] Error in callback:', err);
            }
          }
        )
        .subscribe();
      ...
      console.error('[useScrapeRequestNotifications] Failed to set up subscription:', err);
    }
    ...
    return () => {
      try {
        ...
        console.error('[useScrapeRequestNotifications] Failed to clean up subscription:', err);
      }
    };
  }, []);
}
