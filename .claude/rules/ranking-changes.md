...
`rankings_full` only contains teams that made it through the full pipeline. It cannot be used as a proxy for `global_strength_map` which includes all teams with games. If you need all teams, query the source data, not the output table.
