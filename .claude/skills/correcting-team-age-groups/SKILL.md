...
- **This season's opponents back the current label** — *automatic*: held as
  ...
  their actual age group correct." Sample a few against their schedules, then file them by the
  band. The tool has no flag that applies these, so copy the rows into a plan with `action` set
  to `would_update` and apply that plan the usual way.
