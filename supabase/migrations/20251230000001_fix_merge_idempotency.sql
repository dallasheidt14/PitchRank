-- Migration: Fix merge idempotency and error messages
-- Purpose: Return appropriate messages when merge already exists or team is merged elsewhere
-- This makes the merge operation idempotent (safe to retry)

-- ============================================================================
-- UPDATED EXECUTE TEAM MERGE FUNCTION WITH IDEMPOTENCY
-- ============================================================================
...
BEGIN
    -- 1. Check if team is already deprecated
    IF EXISTS (
        SELECT 1 FROM teams
        WHERE team_id_master = p_deprecated_team_id
        AND is_deprecated = TRUE
    ) THEN
        -- Check if already merged to the SAME canonical team (idempotent case)
        SELECT m.canonical_team_id, t.team_name
        INTO v_existing_canonical, v_existing_canonical_name
        FROM team_merge_map m
        JOIN teams t ON m.canonical_team_id = t.team_id_master
        WHERE m.deprecated_team_id = p_deprecated_team_id;

        IF v_existing_canonical = p_canonical_team_id THEN
            -- Already merged to same team - return success (idempotent)
            RETURN jsonb_build_object(
                'success', TRUE,
                'already_merged', TRUE,
                'canonical_team_id', p_canonical_team_id,
                'message', format('Team is already merged to %s', v_existing_canonical_name)
            );
        ELSIF v_existing_canonical IS NOT NULL THEN
            -- Merged to a DIFFERENT team - provide guidance
            RETURN jsonb_build_object(
                'success', FALSE,
                'error', format('Team is already merged to "%s" (%s). To change the canonical team, update the existing merge record.',
                               v_existing_canonical_name, v_existing_canonical),
                'current_canonical_id', v_existing_canonical,
                'current_canonical_name', v_existing_canonical_name
            );
        ELSE
            -- Deprecated but no merge record (orphaned) - this shouldn't happen
            RETURN jsonb_build_object(
                'success', FALSE,
                'error', format('Team %s is deprecated but has no merge record. This is an orphaned team that needs manual repair.', p_deprecated_team_id)
            );
        END IF;
    END IF;
...
COMMENT ON FUNCTION execute_team_merge IS
    ...
    'Idempotent: if merge already exists to same canonical, returns success. '
    'Supports CASCADE: if other teams are merged into the deprecated team, they are '
...
BEGIN
    ...
    RAISE NOTICE 'Migration successful: execute_team_merge now handles idempotent merges';
END $$;
