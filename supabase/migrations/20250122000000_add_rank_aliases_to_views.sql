-- Add rank_in_cohort_final and rank_in_state_final aliases to views
-- These are the field names expected by the frontend
...
-- =====================================================
-- Step 2: Recreate rankings_view with rank aliases
-- =====================================================
...
SELECT 
    ...
    COALESCE(rf.goals_for, cr.goals_for) as goals_for,
    
    -- Strength of Schedule (multiple SOS values)
    COALESCE(rf.strength_of_schedule, rf.sos, cr.strength_of_schedule) as strength_of_schedule,
    COALESCE(rf.sos, cr.strength_of_schedule) as sos,  -- Raw SOS value (0.0-1.0)
    rf.sos_norm as sos_norm,  -- Normalized SOS (percentile/z-score within cohort)
    
    -- Dynamically calculated ranks
    -- National rank: within age_group + gender cohort, ordered by power_score_final DESC
    -- Frontend expects rank_in_cohort_final (same as national_rank)
    row_number() OVER (
...
COMMENT ON VIEW rankings_view IS 'National rankings view reading from rankings_full (with fallback to current_rankings). Includes power_score_final with ML > global > adj > national fallback. Exposes both raw sos and normalized sos_norm. Includes rank_in_cohort_final alias for frontend compatibility. Respects RLS policies.';

-- =====================================================
-- Step 3: Recreate state_rankings_view with rank aliases
-- =====================================================
...
SELECT
    ...
    -- Frontend expects rank_in_state_final (same as state_rank)
    row_number() OVER (
...
COMMENT ON VIEW state_rankings_view IS 'State rankings view: National rankings filtered by state_code, with dynamically calculated state_rank and state_sos_rank. Includes rank_in_state_final alias for frontend compatibility. Exposes both raw sos and normalized sos_norm. Reads from rankings_full via rankings_view. Respects RLS policies.';
