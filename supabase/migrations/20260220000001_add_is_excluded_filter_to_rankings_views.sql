-- Update rankings_view and state_rankings_view to exclude is_excluded games
-- from total_games_played, total_wins, total_losses, total_draws, win_percentage
-- Date: 2026-02-20
...
-- =====================================================
-- Step 2: Recreate rankings_view with is_excluded filter
-- =====================================================
...
SELECT
    ...
    -- Total wins/losses/draws from ALL non-excluded games (for accurate win% calculation)
    (SELECT COUNT(*)
     FROM games g
...
COMMENT ON VIEW rankings_view IS 'National rankings view using rankings_full as primary source. Filters is_excluded games from total counts. Uses pre-computed rank_in_cohort_ml instead of computing dynamically for performance. Exposes perf_centered for insights form signal. Respects RLS policies.';
...
COMMENT ON VIEW state_rankings_view IS 'State rankings view: National rankings filtered by state_code, with dynamically calculated state_rank. Filters is_excluded games from total counts. Exposes perf_centered for insights. Respects RLS policies.';
