-- Add sos_norm_state to rankings views
-- Date: 2025-01-25
-- Purpose: Expose sos_norm_state (state-normalized SOS) for state rankings display
--          When viewing state rankings, use sos_norm_state instead of sos_norm
--          to show SOS normalized within the state, not nationally
...
-- =====================================================
-- Step 2: Recreate rankings_view with sos_norm_state
-- =====================================================
...
SELECT
    ...
    -- National rank (computed live in view)
    ROW_NUMBER() OVER (
        PARTITION BY 
            CASE
              WHEN rf.age_group ~ '^[0-9]+$' THEN rf.age_group::INTEGER
...
COMMENT ON VIEW rankings_view IS 'National rankings view using rankings_full as primary source. Exposes sos_norm (national) and sos_norm_state (state-normalized) for proper display in state vs national views. Respects RLS policies.';

-- =====================================================
-- Step 3: Recreate state_rankings_view with sos_norm_state
-- =====================================================
...
COMMENT ON VIEW state_rankings_view IS 'State rankings view: National rankings filtered by state_code, with dynamically calculated state_rank. Exposes sos_norm (national) and sos_norm_state (state-normalized). Frontend should use sos_norm_state for state rankings display. Respects RLS policies.';
