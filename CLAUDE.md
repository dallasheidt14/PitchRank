...
- **A Supabase fake must evaluate its filters, or it proves nothing about a write.**
  A double that records `update()` and returns a canned row without applying the
  `eq`/`in_`/`is_` it was handed is strictly more permissive than PostgREST, so an
  unfiltered `UPDATE teams`, a filter on the wrong column, and a permanently-false
  predicate all pass green. Three separate revisions of one suite were caught this
  way, each on a different axis, because each fix pinned the named instance instead
  of the property. Make the double filter, honour `order` before `range`, return the
  rows an update actually matched, and raise on a table nobody seeded — then a
  wrongly-scoped query fails in the test for the same reason it would in production.
...
either, and a provider that says U12 where we say u13 is not on its own evidence
...
authored. On a live AZ u13 slice, all seven rows GotSport filed as U12 played u13
opposition across 243 games — the stored cohort was right and the provider's was
not, which no amount of reading either label would have shown.
...
update when it is wrong, whereas writing on one costs a mislabelled team. IMP-145
bars the second; the first is what any tool reading a provider's cohort should do
with a disagreement.
...
**A release back to `pending` can be refused, and the refusal is swallowed.**
`idx_scrape_requests_pending_team` is `UNIQUE (team_id_master) WHERE status = 'pending'`, and
the daily enqueues (`enqueue-yesterday-games` 07:13 UTC, `enqueue-active-teams` 10:28 UTC) add
fresh pending rows for teams a long run is still holding in `processing`. Releasing such a row
then raises `23505`. `_release_queue_items` updates 100 ids per statement and logs the
exception as a warning, so **one conflicting row strands the other 99** in `processing`. Any
new path that releases claims should go row by row and, on a conflict, retire the redundant
claim to `failed` — the team already has a live pending row, so the work is not lost. The
longer a run holds claims, the likelier it crosses one of those cron times.
