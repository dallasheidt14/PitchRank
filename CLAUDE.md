...
- In a multi-stage chain, confirm which stage actually performs the operation before fixing it anywhere. The stages here are near-identical copies, and the flags differ: `unknown-opponent-hygiene-weekly.yml` passes `--resolve-gotsport-details` to `export_unknown_opponents.py` alone. Three correct fixes landed in the wrong stage and changed nothing in production until the export stage was found. Read the workflow's arguments, not just the scripts.
  - **But the flag is not the only way a resolver gets built, and this paragraph used to claim it was.** Only `export_unknown_opponents.py:299` and `auto_match_unknown_opponents.py:446` gate on `args.resolve_gotsport_details`; `discover_teams_from_opponents.py:543` and `due_diligence_unknown_opponents.py:282` call `GotSportResolver()` unconditionally and so hit GotSport on every weekly run. Verified 2026-09-08. Grep for the constructor across `scripts/`, never for the flag — a flag-shaped audit misses two of the four callers and understates what the weekly run reaches.
