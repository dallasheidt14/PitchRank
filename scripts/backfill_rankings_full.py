...
async def backfill_rankings_full():
    ...
    try:
        ...
        rankings_full_df = current_rankings_df.merge(
            teams_meta_df, left_on="team_id", right_on="team_id_master", how="left"
        )
        ...
        for _, row in rankings_full_df.iterrows():
            record = {
                ...
                "global_power_score": float(row["global_power_score"])
                if pd.notna(row.get("global_power_score"))
                else None,
                ...
                "strength_of_schedule": float(row["strength_of_schedule"])
                if pd.notna(row.get("strength_of_schedule"))
                else None,
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                result = supabase.table("rankings_full").upsert(batch, on_conflict="team_id").execute()
