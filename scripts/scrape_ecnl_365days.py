...
from rich.progress import BarColumn, Progress, SpinnerColumn, TextColumn, TimeRemainingColumn  # noqa: E402

from src.base import GameData  # noqa: E402
from src.scrapers.athleteone_scraper import AthleteOneScraper  # noqa: E402
...
def scrape_ecnl_365days(
    conferences_file: str = None, output_file: str = None, auto_import: bool = False, days_back: int = 365
):
    """
    ...
    console.print(
        Panel.fit(
            f"[bold cyan]ECNL 365-Day Scraper[/bold cyan]\nScraping games from the last {days_back} days",
            border_style="cyan",
        )
    )
    ...
    with Progress(
        ...
        task = progress.add_task(f"Scraping {len(conferences)} conferences...", total=len(conferences))
        ...
        for conf in conferences:
            ...
            try:
                ...
                if filtered_games:
                    ...
                    successful += 1
                    progress.update(
                        task, advance=1, description=f"[green]✓ {display_name}: {len(filtered_games)} games[/green]"
                    )
                else:
                    progress.update(task, advance=1, description=f"[dim]{display_name}: No games[/dim]")

            except Exception as e:
                failed += 1
                logger.error(f"Error scraping {display_name}: {e}", exc_info=True)
                progress.update(task, advance=1, description=f"[red]✗ {display_name}: Error[/red]")
                continue
...
Examples:
  ...
  python scripts/scrape_ecnl_365days.py --days-back 180
    ...
    parser.add_argument("--output", type=str, default=None, help="Output file path (default: auto-generated timestamp)")

    parser.add_argument(
        "--auto-import", action="store_true", help="Automatically import games to database after scraping"
    )
