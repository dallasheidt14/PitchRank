...
def merge_teams(deprecated_team_id: str, canonical_team_id: str, merged_by: str = "script", merge_reason: str = None):
    ...
    deprecated_team = (
        ...
        .select("team_id_master, team_name, club_name, age_group, gender, is_deprecated")
        .eq("team_id_master", deprecated_team_id)
        .maybe_single()
        .execute()
    )
    canonical_team = (
        supabase.table("teams")
        .select("team_id_master, team_name, club_name, age_group, gender, is_deprecated")
        .eq("team_id_master", canonical_team_id)
        .maybe_single()
        .execute()
    )
    ...
    print(
        f"  Deprecated: {deprecated_team.data['team_name']} "
        f"({deprecated_team.data.get('club_name', 'N/A')}) - "
        f"{deprecated_team.data.get('age_group', 'N/A')} {deprecated_team.data.get('gender', 'N/A')}"
    )
    print(
        f"  Canonical: {canonical_team.data['team_name']} "
        f"({canonical_team.data.get('club_name', 'N/A')}) - "
        f"{canonical_team.data.get('age_group', 'N/A')} {canonical_team.data.get('gender', 'N/A')}"
    )
    print()

    if deprecated_team.data.get("is_deprecated"):
        print("⚠️  Warning: Deprecated team is already marked as deprecated")

    if canonical_team.data.get("is_deprecated"):
        print("❌ Error: Cannot merge into a deprecated team!")
        return False
        ...
        if result.data:
            if isinstance(result.data, dict):
                if result.data.get("success"):
                    print("✅ Successfully merged teams!")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("\nExample:")
        print(
            "  python scripts/merge_teams.py ec7b21f9-ef74-441a-94b9-ce70d5d39343 78748e84-900b-4b52-943d-c47687cda27a"
        )
        sys.exit(1)
