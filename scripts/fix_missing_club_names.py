...
def analyze_empty_club_names(state_code: str = None, limit: int = None):
    ...
    # Analyze each state
    results = {"fixable": [], "new_clubs": [], "unfixable": []}
    ...
    for state, teams in sorted(by_state.items(), key=lambda x: -len(x[1])):
        ...
        for t in teams:
            ...
            if matched in existing_clubs:
                results["fixable"].append({**t, "suggested_club": matched, "extracted": extracted})
            else:
                results["new_clubs"].append({**t, "suggested_club": matched, "extracted": extracted})
...
def execute_fixes(results: dict, dry_run: bool = True):
    ...
    for r in results["fixable"]:
        if dry_run:
            ...
            try:
                supabase.table("teams").update({"club_name": r["suggested_club"]}).eq(
