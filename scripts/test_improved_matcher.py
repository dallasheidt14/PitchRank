...
def run_test(limit: int = 25):
    ...
    query = (
        db.table("team_match_review_queue")
        .select("provider_team_name, match_details, suggested_master_team_id")
        .eq("status", "approved")
        ...
        if match is None:
            ...
            if len(correct_examples) < 3:
                correct_examples.append(
                    {"team_name": team_name, "matched": match["team_name"], "confidence": match["confidence"]}
                )
        else:
            wrong += 1
