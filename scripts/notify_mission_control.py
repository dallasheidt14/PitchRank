...
Usage from agent sessions:
    ...
    # On progress update
    python3 scripts/notify_mission_control.py progress "session-key-here" \\
        "codey" "Progress update" --result "50% complete"

    # On completion
    python3 scripts/notify_mission_control.py complete "session-key-here" \\
        "codey" "Done" --result "Feature X implemented successfully"

    # On error
    python3 scripts/notify_mission_control.py error "session-key-here" \\
        "codey" "Failed" --error "Build failed: missing dependency"
...
WEBHOOK_URL = os.environ.get("MISSION_CONTROL_WEBHOOK_URL", "http://localhost:3000/api/agent-webhook")
...
Environment Variables:
  ...
  AGENT_WEBHOOK_SECRET         Optional auth secret
    ...
    parser.add_argument("action", choices=["spawn", "progress", "complete", "error"], help="Action type")
    parser.add_argument("session_key", help="Unique session identifier")
    parser.add_argument("agent_name", help="Agent name (e.g., codey, movy, watchy)")
    parser.add_argument("task", help="Task description")
    parser.add_argument("--result", help="Result message (for progress/complete actions)")
    parser.add_argument("--error", help="Error message (for error action)")
