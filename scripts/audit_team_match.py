...
for i, csv_row in enumerate(selected_teams, 1):
    ...
    alias_result = (
        ...
        .eq("provider_team_id", team_id_tgs)
        .execute()
    )
    ...
    if alias_result.data:
        ...
        team_result = supabase.table("teams").select("*").eq("team_id_master", team_id_master).execute()
        ...
        if team_result.data:
            ...
            if team.get("provider_team_id") == team_id_tgs:
                print(
                    f"\n✅ DIRECT ID MATCH: Master team's provider_team_id "
                    f"({team.get('provider_team_id')}) matches TGS team ID ({team_id_tgs})"
                )
            else:
                print(
                    f"\n⚠️  NO DIRECT ID MATCH: Master team's provider_team_id "
                    f"({team.get('provider_team_id')}) != TGS team ID ({team_id_tgs})"
                )
            ...
            if age_match and gender_match:
                print("✅ Age Group & Gender Match")
            else:
                print(
                    f"⚠️  Mismatch - CSV: age_group={age_group}, gender={gender} "
                    f"| DB: age_group={team.get('age_group')}, gender={team.get('gender')}"
                )
        else:
