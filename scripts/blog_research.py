...
def state_research(state_code: str, age_group: str = None):
    ...
    results = {"state": state_code, "generated_at": datetime.now().isoformat(), "stats": {}}
    ...
    results["stats"]["top_clubs"] = [{"name": row[0], "teams": row[1]} for row in cur.fetchall()]
...
def national_research():
    ...
    results = {"scope": "national", "generated_at": datetime.now().isoformat(), "stats": {}}
    ...
    cur.execute("""
        ...
        LIMIT 15
    """)
    results["stats"]["teams_by_state"] = [{"state": row[0], "teams": row[1]} for row in cur.fetchall()]
    ...
    results["stats"]["games_by_month"] = [
        {"month": row[0].strftime("%Y-%m") if row[0] else None, "games": row[1]} for row in cur.fetchall()
    ]
...
def club_research(club_name: str = None):
    ...
    results = {"topic": "club_analysis", "generated_at": datetime.now().isoformat(), "stats": {}}
    ...
    cur.execute("""
        ...
        LIMIT 25
    """)
    results["stats"]["largest_clubs"] = [{"name": row[0], "state": row[1], "teams": row[2]} for row in cur.fetchall()]
    ...
    results["stats"]["full_pathway_clubs"] = [
        {"name": row[0], "age_groups": row[1], "teams": row[2]} for row in cur.fetchall()
    ]
...
def competitor_urls(keyword: str):
    ...
    competitors = [
        "https://www.gotsoccer.com/rankings",
        "https://www.topdrawersoccer.com/club-soccer-rankings",
        "https://www.soccerwire.com/rankings",
    ]
