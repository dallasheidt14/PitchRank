...
def get_team_movement_analysis(cur, team_id, days=7):
    ...
    cur.execute(
        ...
        LEFT JOIN current_rankings cr_opp ON
            CASE WHEN g.home_team_master_id = %s THEN g.away_team_master_id
                 ELSE g.home_team_master_id END = cr_opp.team_id
...
def generate_commentary(analysis, team_name, club_name):
    ...
    if games:
        ...
        if rank_change > 100:
            if quality_wins:
                explanations.append("Big wins against ranked opponents boosted their strength of schedule")
            ...
            if bad_losses:
                explanations.append("Losses to lower-ranked teams hurt their ranking")
