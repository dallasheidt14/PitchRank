...
no_state_result = (
    supabase.table("teams").select("*", count="exact").is_("state", "null").is_("state_code", "null").execute()
)
...
has_state_no_code_result = (
    supabase.table("teams").select("*", count="exact").not_.is_("state", "null").is_("state_code", "null").execute()
)
...
has_code_no_state_result = (
    supabase.table("teams").select("*", count="exact").is_("state", "null").not_.is_("state_code", "null").execute()
)
...
print(
    f"  Teams with partial state info: "
    f"{has_state_no_code_count + has_code_no_state_count:,} "
    f"({(has_state_no_code_count + has_code_no_state_count) / total_teams * 100:.1f}%)"
)
...
if sample_result.data:
    for i, team in enumerate(sample_result.data, 1):
        print(
            f"{i:2d}. {team.get('team_name', 'N/A')[:50]:<50} "
            f"| {team.get('age_group', 'N/A'):<4} | {team.get('gender', 'N/A')}"
        )
