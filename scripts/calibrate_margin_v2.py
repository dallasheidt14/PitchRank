...
def calculate_margin_error(
    params: np.ndarray, raw_df: pd.DataFrame, age_groups: list, current_margin_mults: dict
) -> float:
    """
    ...
    for age_group in age_groups:
        ...
        predicted_margin = (
            age_df["composite_diff"].values * MARGIN_COEFFICIENT * margin_mult * composite_scaling * margin_scale
        )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="data/calibration/margin_parameters_v2.json", help="Output JSON file path"
    )
