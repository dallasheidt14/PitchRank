            ...
            for team in valid_teams:
                    ...
                    if "unique" in error_str or "duplicate" in error_str or "already exists" in error_str:
                        ...
                        try:
                            ...
                            if existing_team.data:
                                master_team_id = existing_team.data[0]["team_id_master"]
                                logger.debug(
                                    f"Team already exists (fallback): {team['team_name']} (ID: {master_team_id})"
                                )
                            else:
                ...
                try:
                    ...
                    if result.data:
                        alias_count += 1
                        logger.info(
                            f"Created direct ID mapping: "
                            f"{self.provider_code}:{team['provider_team_id']} -> {master_team_id}"
                        )
                except Exception as e:
                    ...
                    if "unique" in error_str or "duplicate" in error_str or "already exists" in error_str:
                        ...
                        logger.warning(
                            f"Failed to create mapping for {self.provider_code}:{team['provider_team_id']}: {e}"
                        )
