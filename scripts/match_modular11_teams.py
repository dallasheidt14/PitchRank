...
Usage:
    ...
    python scripts/match_modular11_teams.py \\
        --csv scrapers/modular11_scraper/output/modular11_results_20251203_105706.csv --preview
"""
        ...
        for row in reader:
            ...
            if team_id not in teams:
                teams[team_id] = Modular11Team(
                    provider_team_id=team_id, club_name=club_name, age_groups=set(), divisions=set(), game_count=0
                )
...
def match_teams(
    modular11_teams: Dict[str, Modular11Team], db_teams: List[DatabaseTeam], age_group: str, division: str
) -> List[MatchResult]:
    ...
    for team_id, m11_team in modular11_teams.items():
        ...
        # Fuzzy match
        if db_club_names:
            matches = process.extract(normalized_m11, db_club_names, scorer=fuzz.token_sort_ratio, limit=3)
...
def create_pending_aliases(supabase, provider_id: str, results: List[MatchResult], min_confidence: int = 80):
...
def main():
    ...
    parser.add_argument(
        "--min-confidence", type=int, default=80, help="Minimum confidence for auto-matching (default: 80)"
    )
    ...
    for team_id, team in list(modular11_teams.items())[:5]:
        print(
            f"    ID {team_id}: {team.club_name} | Ages: {', '.join(sorted(team.age_groups))} "
            f"| Divs: {', '.join(sorted(team.divisions))} | Games: {team.game_count}"
        )
    ...
    if args.create_pending:
        ...
        print(
            "   UPDATE team_alias_map SET review_status = 'approved' "
            "WHERE provider_id = '...' AND review_status = 'pending';"
        )
