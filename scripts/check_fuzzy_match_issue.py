        ...
        for alias in alias_result.data:
            ...
            if matched_team.data:
                ...
                if matched_team_id == team_uuid:
                    print("  ✅ MATCHED TO TARGET TEAM!")
                else:
                    print("  ❌ Matched to different team")
...
for team_id in [t["team_id"] for t in eastside_teams_in_csv.values()]:
    ...
    home_games = (
        ...
        .select("game_uid, game_date, home_score, away_score, home_team_master_id")
        .eq("home_provider_id", team_id)
        .eq("provider_id", tgs_provider_id)
        .limit(5)
        .execute()
    )
    ...
    away_games = (
        supabase.table("games")
        .select("game_uid, game_date, home_score, away_score, away_team_master_id")
        .eq("away_provider_id", team_id)
        .eq("provider_id", tgs_provider_id)
        .limit(5)
        .execute()
    )
...
print("should have been matched to TGS teams via fuzzy matching if:")
print("  1. Team names are similar")
print("  2. Age groups match (or are close)")
print("  3. Gender matches")
print("  4. Club names match")
print("\nThe dry run showed 0 games accepted because all were duplicates.")
print("This means the games are already in the database.")
