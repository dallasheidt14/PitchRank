...
for i, team in enumerate(all_teams, 1):
    ...
    for game in all_games:
        ...
        opp_result = (
            supabase.table("teams").select("team_name, age_group, gender").eq("team_id_master", opponent_id).execute()
        )
    ...
    if problematic_games:
        problematic_teams.append({"team": team, "problematic_games": problematic_games})
...
if problematic_teams:
    ...
    for pt in problematic_teams:
        for pg in pt["problematic_games"]:
            ...
            by_age_diff[age_diff].append({"team": pt["team"], "game": pg})
