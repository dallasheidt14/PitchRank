...
def draw_arrow(ax, x1, y1, x2, y2, color="#555555", style="->", lw=1.5, connectionstyle="arc3,rad=0.0"):
...
def draw_section_bg(ax, x, y, w, h, color, label, label_color="#333333"):
    ...
    rect = FancyBboxPatch(
        (x, y), w, h, boxstyle="round,pad=0.008", facecolor=color, edgecolor="#cccccc", linewidth=1, alpha=0.3, zorder=0
    )
    ax.add_patch(rect)
    ax.text(
        x + 0.012,
...
def generate_pipeline_map():
    ...
    ax.text(
        0.5,
        0.99,
        "PitchRank — Complete Pipeline Process Map",
        ha="center",
        va="top",
        fontsize=22,
        fontweight="bold",
    ...
    ax.text(
        0.5,
        0.982,
        "v2.0.4-config-sync  |  Youth Soccer Team Ranking System  |  Data Collection \u2192 Rankings \u2192 Frontend",
        ha="center",
        va="top",
        fontsize=10,
        color="#666666",
    ...
    for i, (color, label) in enumerate(legend_items):
        ...
        rect = FancyBboxPatch(
            (xi, yi), 0.012, 0.007, boxstyle="round,pad=0.002", facecolor=color, edgecolor="none", alpha=0.9, zorder=10
        )
        ax.add_patch(rect)
        ax.text(xi + 0.016, yi + 0.0035, label, ha="left", va="center", fontsize=6.5, color="#333", zorder=10)
    ...
    p1_h = 0.05
    p1_bot = p1_top - p1_h
    draw_section_bg(ax, 0.02, p1_bot, 0.96, p1_h, "#fff3e0", "PHASE 1: SCHEDULING & TRIGGERS (GitHub Actions)")
    ...
    for label, sub, xpos in triggers:
        draw_rounded_box(ax, xpos, box_y, bw, bh, "", C_TRIGGER, fontsize=7, bold=True, text_lines=[label, sub])
    ...
    p2_h = 0.09
    p2_bot = p2_top - p2_h
    draw_section_bg(ax, 0.02, p2_bot, 0.96, p2_h, "#e3f2fd", "PHASE 2: DATA COLLECTION \u2014 Multi-Provider Scraping")
    ...
    for title, detail, xpos in providers:
        draw_rounded_box(ax, xpos, prov_y, pw, ph, "", C_SCRAPE, fontsize=6, text_lines=[title, *detail.split("\n")])

    # Output bar below providers
    out_y = prov_y - 0.022
    draw_rounded_box(
        ax, 0.30, out_y, 0.40, 0.016, "Output: JSONL / CSV files  \u2192  data/raw/", C_DATA, fontsize=7, bold=True
    )
    ...
    p3_bot = p3_top - p3_h
    draw_section_bg(
        ax, 0.02, p3_bot, 0.96, p3_h, "#e8f5e9", "PHASE 3: ETL PIPELINE \u2014 Ingestion, Validation & Deduplication"
    )
    ...
    draw_arrow(ax, 0.03 + step_w + 0.002, steps_y + step_h / 2, 0.205 - 0.002, steps_y + step_h / 2, "#333")
    ...
    draw_arrow(ax, 0.205 + step_w + 0.002, steps_y + step_h / 2, 0.38 - 0.002, steps_y + step_h / 2, "#333")
    ...
    draw_arrow(ax, 0.38 + step_w + 0.002, steps_y + step_h / 2, 0.555 - 0.002, steps_y + step_h / 2, "#333")
    ...
    draw_arrow(ax, 0.555 + step_w + 0.002, steps_y + step_h / 2, 0.73 - 0.002, steps_y + step_h / 2, "#333")
    ...
    p4_h = 0.10
    p4_bot = p4_top - p4_h
    draw_section_bg(ax, 0.02, p4_bot, 0.96, p4_h, "#fce4ec", "PHASE 4: TEAM MATCHING \u2014 3-Tier Identity Resolution")
    ...
    p5_bot = p5_top - p5_h
    draw_section_bg(
        ax, 0.02, p5_bot, 0.96, p5_h, "#f3e5f5", "PHASE 5: RANKING CALCULATION \u2014 v53e Algorithm + ML Layer 13"
    )
    ...
    p6_h = 0.06
    p6_bot = p6_top - p6_h
    draw_section_bg(ax, 0.02, p6_bot, 0.96, p6_h, "#e0f7fa", "PHASE 6: RANKINGS PERSISTENCE & DERIVATION")
    ...
    draw_rounded_box(
        ...
        C_DB,
        fontsize=6,
        text_lines=["rankings_full", "Comprehensive metrics", "All v53e + ML fields"],
    )
    ...
    draw_rounded_box(
        ...
        C_DB,
        fontsize=6,
        text_lines=["current_rankings", "Backward compatibility", "National + State ranks"],
    )
    ...
    draw_rounded_box(
        ...
        C_DB,
        fontsize=6,
        text_lines=["State Rankings", "Group by state_code", "Rank within each state"],
    )
    ...
    draw_rounded_box(
        ...
        C_DB,
        fontsize=6,
        text_lines=["Cross-Age Global Scores", "U10=0.40 \u2192 U19=1.00 anchors", "Enables cross-age SOS"],
    )
    ...
    p7_bot = p7_top - p7_h
    draw_section_bg(
        ax, 0.02, p7_bot, 0.96, p7_h, "#e8eaf6", "PHASE 7: FRONTEND \u2014 Next.js 16 + React 19 + Supabase Client"
    )
    ...
    for title, detail, xpos in fe_items:
        draw_rounded_box(ax, xpos, fy, fw, fh, "", C_FRONTEND, fontsize=5.5, text_lines=[title, *detail.split("\n")])
    ...
    db_h = 0.03
    db_bot = db_top - db_h
    draw_section_bg(ax, 0.02, db_bot, 0.96, db_h, "#fff8e1", "DATABASE: Supabase PostgreSQL")
    ...
    fig.savefig(output_path, dpi=200, bbox_inches="tight", facecolor="#FAFBFC", edgecolor="none", pad_inches=0.3)
    plt.close(fig)
