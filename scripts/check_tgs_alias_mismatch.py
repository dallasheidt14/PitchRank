...
alias_result = (
    ...
    .select("*")
    .eq("provider_id", tgs_provider_id)
    .eq("provider_team_id", tgs_team_id)
    .execute()
)
...
if alias_result.data:
    for alias in alias_result.data:
        ...
        matched_team = supabase.table("teams").select("*").eq("team_id_master", matched_team_id).execute()
    ...
    print("   This means it was never matched during import.")
    print("   It should have been fuzzy matched to:")
...
home_games = (
    ...
    .select("game_uid, game_date, home_team_master_id")
    .eq("home_provider_id", tgs_team_id)
    .eq("provider_id", tgs_provider_id)
    .limit(3)
    .execute()
)
...
away_games = (
    supabase.table("games")
    .select("game_uid, game_date, away_team_master_id")
    .eq("away_provider_id", tgs_team_id)
    .eq("provider_id", tgs_provider_id)
    .limit(3)
    .execute()
)
...
if alias_result.data:
    print("  ✅ Has alias mapping")
    matched_team_id = alias_result.data[0].get("team_id_master")
    if matched_team_id == target_team_uuid:
        print("  ✅ Correctly matched to GotSport team")
    else:
        print(f"  ❌ Matched to wrong team (UUID: {matched_team_id})")
        print(f"  ❌ Should be matched to: {target_team_uuid}")
else:
    print("  ❌ NO alias mapping exists")
    print("  ❌ This means fuzzy matching failed or wasn't attempted")
    print("  ❌ Games may be linked to a different team or not imported")

print("\nWithout an alias mapping, the system cannot:")
print(f"  1. Direct match TGS Team ID {tgs_team_id} to GotSport team")
print("  2. Link games from TGS import to the correct GotSport team")
print("  3. Merge data from both providers for the same team")
