...
def resolve_team_names(supabase, master_ids: set):
    ...
    for i in range(0, len(batch), 100):
        chunk = batch[i : i + 100]
        result = supabase.table("teams").select("team_id_master, team_name").in_("team_id_master", chunk).execute()
        if result.data:
...
def find_perspective_duplicates(supabase, games: list):
    ...
    for game in games:
        ...
        for row in result.data:
            ...
            same_orientation = (
                r_home == home_master and r_away == away_master and r_h_score == home_score and r_a_score == away_score
            )
            swapped_orientation = (
                r_home == away_master and r_away == home_master and r_h_score == away_score and r_a_score == home_score
            )
...
def exclude_games(supabase, game_ids: list):
    ...
    for game_id in game_ids:
        try:
            supabase.table("games").update({"is_excluded": True}).eq("id", game_id).eq("is_excluded", False).execute()
            excluded_count += 1
...
def main():
    parser = argparse.ArgumentParser(description="Exclude games for a team in a date range from rankings")
    parser.add_argument("--team", required=True, help="Team master ID (UUID)")
    parser.add_argument("--start-date", required=True, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", required=True, help="End date (YYYY-MM-DD)")
    parser.add_argument("--execute", action="store_true", help="Actually exclude games (default is dry-run preview)")
    parser.add_argument(
        "--skip-duplicates",
        action="store_true",
    ...
    if args.execute:
        ...
        console.print(
            f"\n[yellow]Dry run — {len(to_exclude)} game(s) would be excluded. Re-run with --execute to apply.[/yellow]"
        )
