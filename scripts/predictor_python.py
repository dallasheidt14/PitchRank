...
def calculate_recent_form(team_id: str, all_games: List[Game], n: int = RECENT_GAMES_COUNT) -> float:
    ...
    team_games = [g for g in all_games if (g.home_team_master_id == team_id or g.away_team_master_id == team_id)]
...
def get_adaptive_weights(power_diff: float) -> Dict[str, float]:
    ...
    transition_progress = (abs_power_diff - SKILL_GAP_THRESHOLDS["MEDIUM"]) / (
