...
class MatchReviewer:
    ...
    def __init__(self):
        ...
        self.stats = {"total": 0, "approved": 0, "rejected": 0, "skipped": 0}

    def review_pending(self, limit: Optional[int] = None):
        """Review pending matches from team_alias_map"""
        console.print(Panel.fit("[bold green]🔍 PitchRank Match Review Queue[/bold green]", style="green"))
        ...
        query = (
            ...
            .select("*, teams!team_id_master(team_name, club_name, state_code, age_group, gender)")
            .eq("review_status", "pending")
            .gte("match_confidence", 0.75)
    ...
    def _review_match(self, match: Dict, current: int, total: int):
        ...
        console.print("\n[bold]Decision:[/bold]")
        decision = Prompt.ask(
            "Approve, Reject, or Skip?", choices=["a", "r", "s", "approve", "reject", "skip"], default="a"
        ).lower()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, help="Limit number of matches to review")
    parser.add_argument("--stats", action="store_true", help="Show statistics about pending matches")
