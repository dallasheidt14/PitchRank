...
def analyze_cohort(df, age_group, gender, top_n=30, sos_threshold=0.35):
    ...
    high_talent_low_sos = top[(top["off_norm"] > 0.70) & (top["sos_norm"] < sos_threshold)]
    ...
    schedule_dodgers = top[(top["win_percentage"] > 0.70) & (top["sos_norm"] < 0.40)]
...
def print_cohort_report(findings):
    ...
    if len(bad) > 0:
        ...
        for _, r in bad.iterrows():
            ...
            print(
                f"  {int(r[rank_col]):<6} {str(r['team_id']):<40} "
    ...
    if len(dodgers) > 0:
        ...
        for _, r in dodgers.iterrows():
            ps = r.get("powerscore_ml") or r.get("power_score_final") or r.get("powerscore_adj") or 0
            print(
                f"    Rank {int(r[rank_col])}: PS={ps:.3f}, SOS={r['sos_norm']:.3f}, "
    ...
    print(
        f"  {'Rk':<4} {'PS':>6} {'OFF':>6} {'DEF':>6} {'SOS':>6} {'W%':>6} {'GP':>4} {'W-L-D':>9} {'St':>3} {'Team ID'}"
    )
    print(f"  {'-' * 80}")
    for _, r in top.iterrows():
        ...
        print(
            f"  {int(r[rank_col]):<4} {ps:>6.3f} {r['off_norm']:>6.3f} {r['def_norm']:>6.3f} "
            f"{r['sos_norm']:>6.3f} {r['win_percentage']:>6.1%} {int(r['games_played']):>4} "
...
def main():
    ...
    parser.add_argument("--min-cohort-size", type=int, default=30, help="Skip cohorts smaller than this (default: 30)")
