...
def get_current_team_data(team_id: str) -> dict:
    ...
    try:
        result = (
            ...
            .select("team_id_master, team_name, club_name, state_code, state, age_group, gender, birth_year")
...
def update_teams_from_csv(csv_path: str, dry_run: bool = False, auto_yes: bool = False):
    ...
    if selected_encoding is None:
        console.print("[red]Error: Could not read CSV file with any supported encoding[/red]")
        return
        ...
        for row_num, row in enumerate(reader, start=2):  # Start at 2 (row 1 is header)
            ...
            try:
                alias_result = (
                    ...
                    .select("provider_id, provider_team_id, division")
                    .eq("team_id_master", team_id)
                    .execute()
                )
            ...
            if csv_alias:
                ...
                try:
                    ...
                    if all_aliases_result.data:
                        current_alias_list = [
                            a.get("provider_team_id") for a in all_aliases_result.data if a.get("provider_team_id")
                        ]
                except Exception:
                    pass
            ...
            if not update_payload and not alias_update_payload:
                skipped.append(
                    {"row_num": row_num, "team_name": row.get("team_name", "Unknown"), "reason": "No changes detected"}
                )
                continue
    ...
    console.print(
        Panel(
            ...
            f"Skipped (no changes): "
            f"[yellow]{len([s for s in skipped if s['reason'] == 'No changes detected'])}[/yellow]\n"
    ...
    if errors:
        ...
        for error in errors[:20]:  # Show first 20 errors
            error_table.add_row(
                str(error["row_num"]), error["team_id"][:8] + "...", error["team_name"][:40], error["reason"]
            )
    ...
    if updates:
        ...
        for update in updates[:10]:
            changes_str = ", ".join([f"{c['field']}: {c['old']} → {c['new']}" for c in update["changes"]])
            preview_table.add_row(update["team_name"][:40], changes_str)
        ...
        for update in batch:
            try:
                ...
                if update["update_payload"]:
                    result = (
                        supabase.table("teams")
                        .update(update["update_payload"])
                        .eq("team_id_master", update["team_id"])
                        .execute()
                    )
                ...
                if update["alias_update_payload"]:
                    ...
                    if "division" in alias_payload:
                        supabase.table("team_alias_map").update({"division": alias_payload["division"]}).eq(
                    ...
                    if "modular11_alias" in alias_payload and modular11_provider_id:
                        ...
                        if existing.data:
                            ...
                            if existing.data[0].get("provider_team_id") != modular11_alias:
                                result = (
                                    supabase.table("team_alias_map")
                                    .update({"provider_team_id": modular11_alias})
                                    .eq("team_id_master", update["team_id"])
                                    .eq("provider_id", modular11_provider_id)
                                    .execute()
                                )
                                ...
                                if not result.data:
                                    console.print(
                                        f"[yellow]Warning: Failed to update modular11_alias "
                                        f"for team {update['team_id']}[/yellow]"
                                    )
                        else:
                            ...
                            try:
                                ...
                                if not result.data:
                                    console.print(
                                        f"[yellow]Warning: Failed to create modular11_alias "
                                        f"for team {update['team_id']}[/yellow]"
                                    )
                                ...
                                if "duplicate key" in str(alias_error).lower() or "23505" in str(alias_error):
                                    console.print(
                                        f"[yellow]Warning: Modular11 alias {modular11_alias} "
                                        f"already exists for another team, skipping[/yellow]"
                                    )
                                else:
                                    raise
                    ...
                    if "alias_all" in alias_payload:
                        ...
                        try:
                            existing_result = (
                                supabase.table("team_alias_map")
                                .select("id, provider_id, provider_team_id")
                                .eq("team_id_master", update["team_id"])
                                .execute()
                            )
                        ...
                        for alias_value in all_aliases:
                            ...
                            try:
                                ...
                                check_result = (
                                    supabase.table("team_alias_map")
                                    .select("provider_id")
                                    .eq("provider_team_id", alias_value)
                                    .limit(1)
                                    .execute()
                                )
                            ...
                            if not provider_id_to_use:
                                if existing_aliases:
                                    ...
                                    console.print(
                                        f"[yellow]Warning: Cannot create alias {alias_value} "
                                        f"- no provider available[/yellow]"
                                    )
                                    continue
                            ...
                            try:
                                ...
                                if not result.data:
                                    console.print(
                                        f"[yellow]Warning: Failed to create alias {alias_value} "
                                        f"for team {update['team_id']}[/yellow]"
                                    )
                            except Exception as alias_error:
                                ...
                                if "duplicate key" in str(alias_error).lower() or "23505" in str(alias_error):
                                    console.print(
                                        f"[yellow]Warning: Alias {alias_value} "
                                        f"already exists for another team, skipping[/yellow]"
                                    )
                                else:
                                    console.print(
                                        f"[yellow]Warning: Error creating alias {alias_value}: {alias_error}[/yellow]"
                                    )
                    ...
                    # Fallback: handle single alias if alias_all not present
                        ...
                        if existing.data:
                            ...
                            if existing.data[0].get("provider_team_id") != generic_alias:
                                result = (
                                    supabase.table("team_alias_map")
                                    .update({"provider_team_id": generic_alias})
                                ...
                                if not result.data:
                                    console.print(
                                        f"[yellow]Warning: Failed to update alias for team {update['team_id']}[/yellow]"
                                    )
                        else:
                            ...
                            if modular11_provider_id:
                                try:
                                    ...
                                    if not result.data:
                                        console.print(
                                            f"[yellow]Warning: Failed to create alias "
                                            f"for team {update['team_id']}[/yellow]"
                                        )
                                except Exception as alias_error:
                                    # Check if it's a duplicate key error
                                    if "duplicate key" in str(alias_error).lower() or "23505" in str(alias_error):
                                        console.print(
                                            f"[yellow]Warning: Alias {generic_alias} "
                                            f"already exists for another team, skipping[/yellow]"
                                        )
                                    else:
                                        raise
    ...
    if skipped:
        [s for s in skipped if s["reason"] == "No changes detected"]
