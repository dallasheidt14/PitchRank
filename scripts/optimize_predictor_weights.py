...
def train_optimal_weights(X: np.ndarray, y: np.ndarray, feature_names: list) -> dict:
    ...
    hand_tuned = {"power_diff": 0.58, "sos_diff": 0.18, "form_diff_norm": 0.20, "matchup_advantage": 0.04}
...
def calculate_optimal_sensitivity(X: np.ndarray, y: np.ndarray, feature_weights: dict) -> float:
    ...
    weights = [
        feature_weights[f]["raw_coefficient"] for f in ["power_diff", "sos_diff", "form_diff_norm", "matchup_advantage"]
    ]
...
def main():
    ...
    try:
        ...
        calibration_results = calibrate_probabilities(weight_results["model"], X, y)
        weight_results["calibration"] = {k: v for k, v in calibration_results.items() if k != "calibrated_model"}

        # Find optimal sensitivity
        optimal_sens = calculate_optimal_sensitivity(X, y, weight_results["weights"])
        weight_results["optimal_sensitivity"] = optimal_sens

        # Generate output
        generate_output(weight_results, output_dir)
