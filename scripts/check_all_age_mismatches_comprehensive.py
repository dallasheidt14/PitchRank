...
modular11_teams_result = (
    supabase.table("teams")
    .select("team_id_master, team_name, age_group, gender, provider_team_id")
...
games_result = (
    ...
    .select("id, game_uid, game_date, home_team_master_id, away_team_master_id, home_score, away_score, provider_id")
    .eq("provider_id", modular11_provider_id)
    .execute()
)
...
if len(problematic_games) > 0:
    ...
    for game in problematic_games[:15]:
        print(
            f"  {game['game_date']}: {game['home_team']} ({game['home_age']}, {game['home_provider']}) "
            f"vs {game['away_team']} ({game['away_age']}, {game['away_provider']})"
        )
