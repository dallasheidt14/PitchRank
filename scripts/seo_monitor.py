...
def check_sitemap():
    ...
    try:
        result = subprocess.run(["curl", "-sL", "https://pitchrank.io/sitemap.xml"], capture_output=True, text=True)
        xml = result.stdout
        url_count = xml.count("<url>")
        return {"available": url_count > 0, "url_count": url_count, "has_lastmod": "<lastmod>" in xml}
    except Exception as e:
        return {"error": str(e)}
...
def check_robots_txt():
    ...
    try:
        result = subprocess.run(["curl", "-sL", "https://pitchrank.io/robots.txt"], capture_output=True, text=True)
