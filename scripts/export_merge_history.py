...
def main():
    ...
    parser.add_argument("--output", "-o", default="data/exports/merge_history.csv", help="Output CSV path")
    parser.add_argument(
        "--limit", "-n", type=int, default=500, help="Max merges to export (default 500). Use 0 for all."
    )
    ...
    while True:
        result = (
            db.table("team_merge_map")
            .select("id, deprecated_team_id, canonical_team_id, merged_at, merged_by, merge_reason")
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "merged_by",
                "merge_reason",
                "deprecated_team_name",
                "deprecated_club_name",
                "deprecated_state",
                "canonical_team_name",
                "canonical_club_name",
                "canonical_state",
    ...
    for i, r in enumerate(rows[:5], 1):
        print(
            f"{i}. {r['deprecated_team_name'][:35]:<35} "
        f"| {r['deprecated_club_name'][:25]:<25} → {r['canonical_club_name'][:25]}"
        )
    return 0
