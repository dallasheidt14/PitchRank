...
for team_info in incorrect_teams:
    ...
    if team_id:
        teams_to_check.append({"team_id": team_id, "team_name": team_name})
...
for i, team in enumerate(teams_to_check, 1):
    ...
    if alias_result.data:
        ...
        # Get master team
        master_result = supabase.table("teams").select("*").eq("team_id_master", alias.get("team_id_master")).execute()
        ...
        if master_result.data:
            ...
            if master.get("provider_team_id") == team_id:
                ...
                print(
                    f"\n⚠️  FUZZY MATCH: Master team's provider_team_id "
                    f"({master.get('provider_team_id')}) != TGS ID ({team_id})"
                )
                ...
                master_has_ecnl_only = (
                    "ECNL" in master_upper and "RL" not in master_upper and "ECRL" not in master_upper
                )
                ...
                if provider_has_ecnl_rl and (master_has_ecnl_only or master_has_ecrl):
                    ...
                    print("   Provider: ECNL RL")
                    print(f"   Master: {'ECNL' if master_has_ecnl_only else 'ECRL/RL'}")
        else:
            print("\n❌ Master team not found!")
    else:
        print("\n❌ NO ALIAS FOUND")
        print("   This team was not matched during re-import")
        print("   It should have been created as a new team")
        ...
        print("   Checking if games exist for this team...")
        ...
        if team_exists.data:
            print("   ⚠️  Team exists in teams table but no alias!")
            for t in team_exists.data:
                print(f"      Team ID: {t.get('team_id_master')}")
                print(f"      Team Name: {t.get('team_name')}")
        else:
            print("   Team does not exist in teams table")
            print("   This team should have been created but wasn't")
