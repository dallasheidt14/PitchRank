...
deprecated_team = (
    ...
    .select("team_id_master, team_name, is_deprecated, club_name, age_group, gender")
    .eq("team_id_master", deprecated_team_id)
    .maybe_single()
    .execute()
)
if deprecated_team.data:
    print("✅ Deprecated Team Found:")
...
merge_audit = (
    ...
    .select("*")
    .eq("deprecated_team_id", deprecated_team_id)
...
if alias_map.data:
    ...
    print("ℹ️  No alias entries found (or already updated)")

# Check if canonical team has aliases
canonical_aliases = (
    supabase.table("team_alias_map").select("*").eq("team_id_master", canonical_team_id).limit(5).execute()
)
