...
from src.scrapers.athleteone_event import AthleteOneEventScraper  # noqa: E402
...
def save_scraped_event(scraped_file: Path, event_key: str):
    ...
    data = {"events": list(scraped_events), "last_updated": datetime.now().isoformat()}
...
def scrape_weekly_games(
        ...
        try:
            ...
            if use_saved_html and html_cache_dir:
                ...
                possible_files = [
                    ...
                    html_cache_path / "athleteone_november_2025.html",  # Generic fallback
            ...
            if filtered_games:
                ...
                console.print("  [dim]No games found in date range[/dim]")
    ...
    if all_games:
        ...
        for game in all_games[:5]:
            ...
            table.add_row(game.team_name or "", game.opponent_name or "", game.game_date or "", score)
...
def main():
    parser = argparse.ArgumentParser(description="Weekly incremental scraper for AthleteOne/TGS events")
    ...
    parser.add_argument(
        "--auto-import", action="store_true", help="Automatically import games to database (not yet implemented)"
    )
    parser.add_argument(
        "--use-saved-html", action="store_true", help="Use saved HTML files from html-cache-dir instead of API calls"
    )
    parser.add_argument(
        "--html-cache-dir",
        type=str,
