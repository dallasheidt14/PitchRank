...
for team in teams_to_check:
    ...
    if alias_result.data:
        ...
        if match_method == "import":
            ...
            master_result = (
                supabase.table("teams").select("team_name").eq("team_id_master", alias.get("team_id_master")).execute()
            )
                ...
                if provider_has_ecnl_rl and not master_has_ecnl_rl:
                    if master_has_ecnl_only:
                        fuzzy_need_review.append(
                            {"team": team["name"], "master": master_name, "issue": "ECNL RL matched to ECNL"}
                        )
                    else:
                        fuzzy_need_review.append(
                            {
...
print("✅ 2 teams fuzzy matched:")
print("   - Rebels SC: Valid match (both ECNL RL)")
print("   - Sporting CA USA G09: Matched to existing team from different provider")
print("     (Team name doesn't include league designation - may be correct)")
