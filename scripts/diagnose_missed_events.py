...
def compare_discovered_vs_scraped(days_back: int = 14, scraped_events_file: str = "data/raw/scraped_events.json"):
...
def check_recent_summaries(days_back: int = 7):
    ...
    console.print("[cyan]Checking recent summary files for issues...[/cyan]")
    ...
    for summary_file in recent_summaries:
        try:
            ...
            table.add_row(
                date_str, str(len(events)), str(success_count), str(no_teams_count), str(error_count), str(total_games)
            )
...
def main():
    ...
    parser.add_argument(
        "--days-back", type=int, default=14, help="How many days back to check for missed events (default: 14)"
    )
    parser.add_argument(
        "--scraped-events", type=str, default="data/raw/scraped_events.json", help="Path to scraped_events.json file"
    )
    parser.add_argument("--check-summaries", action="store_true", help="Also check recent summary files for issues")
    ...
    missed_events = compare_discovered_vs_scraped(days_back=args.days_back, scraped_events_file=args.scraped_events)
