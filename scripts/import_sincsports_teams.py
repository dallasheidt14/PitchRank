    ...
    try:
        ...
        if result.data:
            ...
            if result.data:
                ...
                console.print("  [red]✗[/red] Failed to create provider")
                return None
...
async def import_teams(team_ids: list, dry_run: bool = False):
    ...
    for team_id in team_ids:
        try:
            ...
            if not is_valid:
                console.print(
                    f"  [yellow]⚠[/yellow] Team {team_id} ({team_data['team_name']}): Validation failed: {errors}"
                )
            ...
            try:
                ...
                if result.data:
                    ...
                    console.print(
                        f"  [dim]✓[/dim] Team {team_id}: {team_data['team_name']} (already exists from SincSports)"
                    )
                ...
                if team_data.get("team_name") and team_data.get("age_group") and team_data.get("gender"):
                    ...
                    if match_result.get("matched"):
                        ...
                        console.print(
                            f"  [green]✓[/green] Team {team_id}: {team_data['team_name']} "
                            f"(matched via {method_label}, confidence: {confidence:.1%})"
                        )
    ...
    if dry_run:
        ...
        if teams_fuzzy_matched:
            ...
            for team in teams_fuzzy_matched:
                ...
                table.add_row(team["team_id"], team["team_name"][:30], method_label, f"{team['confidence']:.1%}")
            console.print(table)
        if teams_to_create:
            console.print("\nTeams to create:")
            table = Table(box=box.SIMPLE)
    ...
    for matched in teams_fuzzy_matched:
        try:
            alias_check = (
                ...
                .select("id")
                .eq("provider_id", provider_id)
                .eq("provider_team_id", matched["team_id"])
                .execute()
            )
    ...
    for existing in teams_existing:
        try:
            ...
            alias_check = (
                ...
                .select("id")
                .eq("provider_id", provider_id)
                .eq("provider_team_id", existing["team_id"])
                .execute()
            )
        ...
        try:
            ...
            if result.data:
                ...
                try:
                    ...
                    error_str = str(e).lower()
                    if "unique" in error_str or "duplicate" in error_str:
                        console.print("    [dim]Mapping already exists[/dim]")
                    else:
