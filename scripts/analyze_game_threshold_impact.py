...
def main():
    ...
    while True:
        result = (
            ...
            .select("games_played, status, age, gender, state")
    ...
    if len(teams_with_4) > 0:
        ...
        print("-" * 70)
        state_breakdown = (
            would_become_active[would_become_active["state"].notna()]
            .groupby("state")
            .size()
    ...
    df_projected["projected_status"] = df_projected.apply(
        lambda row: "Active" if row["games_played"] >= 4 and row["status"] != "Inactive" else row["status"], axis=1
    )
    ...
    print(
        f"Decrease: "
        f"{projected_status.get('Not Enough Ranked Games', 0) - current_status.get('Not Enough Ranked Games', 0):,}"
        f" teams"
    )
    print()
