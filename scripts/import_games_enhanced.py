...
def stream_games_csv(
    file_path: Path, batch_size: int = 1000, limit: Optional[int] = None
...
def log_checkpoint(batch_num: int, games_processed: int, elapsed_time: float, provider: str):
    ...
    with open(log_dir / "import_progress.log", "a") as f:
        f.write(
            f"{datetime.now()} - Provider: {provider} - Batch {batch_num} - "
            f"Games: {games_processed:,} - Elapsed: {elapsed_time:.1f}s\n"
        )


async def import_batch_with_retry(
    pipeline: EnhancedETLPipeline, batch: List[Dict], semaphore: asyncio.Semaphore, max_retries: int = 2
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Batch size for streaming and inserts (default: 1000, recommended: 500 for Supabase)",
    )
    ...
    parser.add_argument(
        "--summary-only", action="store_true", help="Suppress per-team logs and show only summary (Modular11)"
    )
        ...
        if games is not None:
            ...
            result = {
                "summary": {
                    ...
                    "validation_rate": len(all_valid) / (len(all_valid) + len(all_invalid))
    ...
    if not supabase_url or not supabase_key:
        console.print(
            "[red]Error: SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set as environment variables[/red]"
        )
    ...
    try:
        if use_streaming and games_batches is not None:
            ...
            logger.info(
                f"Starting streaming {mode_text.lower()} for provider: {args.provider}, "
            ...
            async def process_batch(batch_num: int, batch: List[Dict]) -> Optional[ImportMetrics]:
                ...
                try:
                    ...
                    failed_batches.append({"batch_num": batch_num, "size": len(batch), "error": str(e)})
            ...
            for batch in games_batches:
                ...
                if len(pending_tasks) >= args.concurrency * 2:
                    ...
                    done, pending_tasks_set = await asyncio.wait(pending_tasks, return_when=asyncio.FIRST_COMPLETED)
                    ...
                    for task in done:
                        ...
                        if result:
                            ...
                            if args.checkpoint and completed_batches % 10 == 0:
                                ...
                                log_checkpoint(
                                    completed_batches, aggregated_metrics.games_processed, elapsed, args.provider
                                )
                                logger.info(
                                    f"Checkpoint: {completed_batches} batches completed, "
            ...
            logger.info(
                f"Starting {mode_text.lower()} for provider: {args.provider}, "
        ...
        logger.info(
            f"Import completed for provider: {args.provider}, "
        ...
        if aggregated_metrics.duplicates_skipped > 0:
            dedup_rate = (
                (aggregated_metrics.duplicates_skipped / aggregated_metrics.games_processed * 100)
                if aggregated_metrics.games_processed > 0
                else 0
            )
            console.print(
                f"  [yellow]Duplicates skipped: {aggregated_metrics.duplicates_skipped:,}[/yellow] ({dedup_rate:.1f}%)"
            )
