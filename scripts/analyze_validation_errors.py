...
def validate_game_from_csv_row(row: Dict) -> tuple:
    ...
    try:
        ...
        if game["goals_against"]:
            game["goals_against"] = (
                float(game["goals_against"]) if "." in str(game["goals_against"]) else int(game["goals_against"])
            )
...
def analyze_validation_errors(csv_file: str, limit: int = None):
            ...
            if is_valid:
                ...
                for error in errors:
                    ...
                    if len(error_examples[error]) < 3:
                        error_examples[error].append({"row": row_num, "game": game, "all_errors": errors})
    ...
    if error_counts:
        ...
        for error, count in error_counts.most_common():
            ...
            table.add_row(error, f"{count:,}", f"{percentage:.1f}%")
