...
async def weekly_update(
    ...
    console.print(Panel.fit("[bold green]🔄 PitchRank Weekly Update[/bold green]", style="green"))
    console.print(f"[dim]Started at: {start_time.strftime('%Y-%m-%d %H:%M:%S')}[/dim]\n")

    results = {"scrape": None, "import": None, "rankings": None}
    ...
    all_succeeded = (
        all(v for v in results.values() if v is not None) if any(v is not None for v in results.values()) else False
    )
...
Examples:
  ...
  python scripts/weekly/update.py --no-ml
    ...
    parser.add_argument("--provider", type=str, default="gotsport", help="Provider code (default: gotsport)")
    parser.add_argument("--games-file", type=str, default=None, help="Path to games CSV file (for import step)")
    parser.add_argument("--ml", action="store_true", default=True, help="Use ML-enhanced rankings (default: True)")
    parser.add_argument("--no-ml", action="store_true", help="Use v53e-only rankings (no ML)")
    ...
    parser.add_argument("--skip-rankings", action="store_true", help="Skip rankings calculation")
