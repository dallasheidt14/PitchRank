...
def check_scraper_health(cur):
    ...
    # Scrape watermarks
    cur.execute(
        "SELECT provider, last_successful_scrape FROM scrape_watermarks ORDER BY last_successful_scrape DESC LIMIT 5"
    )
...
def main():
    ...
    parser.add_argument(
        "--preflight", action="store_true", help="Quick check: exit 0 if OK (skip agent), exit 1 if needs attention"
    )
