...
def save_scraped_events(file_path: Path, event_ids: set, last_updated: str = None):
    ...
    with open(file_path, "w") as f:
        json.dump(
            {"scraped_event_ids": sorted(list(event_ids)), "last_updated": last_updated or datetime.now().isoformat()},
            f,
            indent=2,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file",
        type=str,
        default="data/raw/scraped_events.json",
