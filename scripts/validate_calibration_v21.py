#!/usr/bin/env python3
"""
Validation Script for Match Predictor v2.2 Calibration Changes
...
def test_probability_calibration():
    ...
    print(
        f"\n{'Bucket':<12} {'Games':>10} {'Predicted':>10} {'Actual':>10} "
        f"{'Old Err':>10} {'New Pred':>10} {'New Err':>10}"
    )
    print("-" * 70)
    ...
    for bucket_name, data in bucket_data.items():
        ...
        print(
            f"{bucket_name:<12} {games:>10,} {predicted:>10.1%} {actual:>10.1%} "
            f"{old_error:>10.1%} {new_predicted:>10.1%} {new_error:>10.1%} {improvement}"
        )
...
def test_draw_threshold():
    ...
    print("  Note: Draw threshold helps when actual outcome IS a draw")
...
def test_confidence_params():
    ...
    for key, value in weights.items():
        ...
        intuitive = (
            "✓"
            if (key == "composite_diff" and value > 0)
            or (key == "variance" and value < 0)
            or (key == "sample_strength" and value > 0)
            else "?"
        )
...
def estimate_accuracy_improvement():
    ...
    print("  Draw threshold bonus: ~+0.5-1.0%")
    print("  Margin fix bonus:     ~+0.3-0.5%")
