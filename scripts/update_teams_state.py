...
def update_teams_from_csv(csv_path: str, auto_yes: bool = False):
    ...
    for i, update in enumerate(updates[:10], 1):
        print(
            f"{i:2d}. {update['team_name'][:50]:<50} "
            f"| State: {update['state'] or 'N/A':<20} | Code: {update['state_code'] or 'N/A'}"
        )
        ...
        for update in batch:
            try:
                ...
                result = (
                    ...
                    .update({"state": update["state"], "state_code": update["state_code"]})
                    .eq("team_id_master", update["team_id_master"])
                    .execute()
                )
