...
def fetch_teams(
    ...
    query = supabase.table("teams").select(
        ...
        "state_code, state, birth_year, is_deprecated, provider_id"
    )
    ...
    for i in range(0, len(team_ids), batch_size):
        ...
        try:
            alias_result = (
                ...
                .select("team_id_master, provider_team_id, division, provider_id")
...
def display_teams(teams_df: pd.DataFrame, rankings_df: pd.DataFrame = None, title: str = "Teams"):
    ...
    if rankings_df is not None and not rankings_df.empty:
        teams_df = teams_df.merge(rankings_df, left_on="team_id_master", right_on="team_id", how="left")

    # Sort by national_rank if available, otherwise by team_name
    if "national_rank" in teams_df.columns:
        teams_df = teams_df.sort_values(by=["national_rank", "team_name"], ascending=[True, True], na_position="last")
    else:
        ...
        if "national_rank" in teams_df.columns:
            ...
            row_data.extend(
                [
                    ...
                    (
                        f"{int(wins) if pd.notna(wins) else 0}-"
                        f"{int(losses) if pd.notna(losses) else 0}-"
                        f"{int(draws) if pd.notna(draws) else 0}"
                    ),
                ]
            )
...
def export_to_csv(teams_df: pd.DataFrame, rankings_df: pd.DataFrame, filepath: str):
    ...
    # Merge rankings if available
    if rankings_df is not None and not rankings_df.empty:
        export_df = teams_df.merge(rankings_df, left_on="team_id_master", right_on="team_id", how="left")
    else:
        export_df = teams_df.copy()

    # Sort by national_rank if available
    if "national_rank" in export_df.columns:
        export_df = export_df.sort_values(by=["national_rank", "team_name"], ascending=[True, True], na_position="last")
...
Examples:
  ...
  python scripts/view_teams.py --age-group u13 --gender Male --state AZ --with-rankings
    ...
    parser.add_argument(
        "--gender", "-g", type=str, choices=["Male", "Female", "male", "female"], help="Gender (Male or Female)"
    )
    parser.add_argument("--state", "-s", type=str, help="State code (e.g., AZ, CA, TX)")
    parser.add_argument("--no-state", action="store_true", help="Filter for teams with no state code")
    parser.add_argument(
        "--modular11-or-division", action="store_true", help="Only include teams with Modular11 alias or AD/HD division"
    )
    parser.add_argument(
        "--with-rankings", "-r", action="store_true", help="Include rankings data (national rank, power score, etc.)"
    )
    parser.add_argument("--include-deprecated", action="store_true", help="Include deprecated/merged teams")
    parser.add_argument("--export", "-e", type=str, metavar="FILE", help="Export results to CSV file")
    parser.add_argument("--limit", "-l", type=int, default=None, help="Limit number of teams displayed")
    ...
    if args.limit:
        if not rankings_df.empty:
            ...
            merged = teams_df.merge(rankings_df, left_on="team_id_master", right_on="team_id", how="left").sort_values(
                "national_rank", na_position="last"
            )
