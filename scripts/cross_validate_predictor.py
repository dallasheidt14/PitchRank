...
def split_time_windows(games_df: pd.DataFrame, n_folds: int = 5) -> list:
    ...
    for fold_idx in range(n_folds):
        ...
        train_df = games_df[(games_df["game_date"] >= min_date) & (games_df["game_date"] < train_end)].copy()

        test_df = games_df[(games_df["game_date"] >= test_start) & (games_df["game_date"] < test_end)].copy()
...
def calculate_fold_metrics(results_df: pd.DataFrame) -> dict:
    ...
    for age_group, age_df in results_df.groupby("age_group"):
        ...
        by_age[age_group] = {"accuracy": float(age_accuracy), "mae": float(age_mae), "games": int(len(age_df))}
...
async def run_cross_validation(supabase: Client, lookback_days: int = 365) -> dict:
    ...
    for fold_info in folds:
        ...
        results_df = await run_backtest(supabase, test_df, rankings_df, team_histories, output_dir)

        # Calculate metrics
        metrics = calculate_fold_metrics(results_df)
        metrics["fold"] = fold_num
        metrics["test_period"] = {"start": fold_info["test_start"], "end": fold_info["test_end"]}
    ...
    return {"folds": fold_results, "stability": stability}


async def main():
    parser = argparse.ArgumentParser(description="Cross-validate match predictor")
    parser.add_argument("--lookback-days", type=int, default=365, help="Number of days to look back for games")
    parser.add_argument(
        "--output", type=str, default="data/calibration/cross_validation_results.json", help="Output JSON file path"
    )
