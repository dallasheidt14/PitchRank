...
REQUIRED_COLUMNS = {"provider", "team_name", "opponent_name", "game_date", "goals_for", "goals_against"}

# Optional but important columns
IMPORTANT_COLUMNS = {"club_name", "team_club_name", "age_group", "gender", "state", "home_away"}
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for row_num, row in enumerate(reader, 1):
            ...
            try:
                if game["goals_for"]:
                    game["goals_for"] = (
                        float(game["goals_for"]) if "." in str(game["goals_for"]) else int(game["goals_for"])
                    )
                if game["goals_against"]:
                    game["goals_against"] = (
                        float(game["goals_against"])
                        if "." in str(game["goals_against"])
                        else int(game["goals_against"])
                    )
            ...
            if is_valid:
                ...
                if len(stats["validation_errors"]) < 10:
                    stats["validation_errors"].append({"row": row_num, "game": game, "errors": errors})
...
def check_quarantine_system() -> bool:
    ...
    try:
        ...
        try:
            ...
            supabase.table("quarantine_games").select("*").limit(1).execute()
            return True
        except Exception:
            return False
    except Exception:
...
def verify_pre_import(
    file_path: str, batch_size: int = 2000, concurrency: int = 4, sample_size: int = 10000
) -> VerificationResults:
    ...
    table.add_row(
        "Valid Records", f"{stats['valid_records']:,} ({stats['valid_records'] / stats['total_records'] * 100:.1f}%)"
    )
    table.add_row(
        "Invalid Records",
        f"{stats['invalid_records']:,} ({stats['invalid_records'] / stats['total_records'] * 100:.1f}%)",
    )
    ...
    console.print(
        Panel.fit(
            ...
            f"[bold]Ready for Import: "
            f"{'✅ YES' if all([results.file_format_recognized, results.schema_mapping_correct, batch_ok, concurrency_ok, quarantine_ok]) else '❌ NO - Fix issues above'}"  # noqa: E501  - complex conditional
            "[/bold]",
            title="Final Status",
        )
    )
