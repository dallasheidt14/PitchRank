...
def fetch_team_game_histories(supabase: Client, team_ids: list, lookback_days: int = 365) -> dict:
    """
...
def fit_confidence_weights(raw_df: pd.DataFrame, supabase: Client) -> dict:
    ...
    # Get unique team IDs
    team_ids = list(
        set(list(raw_df["team_a_id"].dropna().astype(str)) + list(raw_df["team_b_id"].dropna().astype(str)))
    )
    ...
    for i in range(0, len(team_ids), batch_size):
        ...
        try:
            rankings_response = (
                supabase.table("rankings_full").select("team_id, games_played").in_("team_id", batch).execute()
            )
    ...
    abs(coefficients[0]) / abs(current_weights["composite_diff"]) if coefficients[0] != 0 else 1.0
    abs(coefficients[1]) / abs(current_weights["variance"]) if coefficients[1] != 0 else 1.0
    abs(coefficients[2]) / abs(current_weights["sample_strength"]) if coefficients[2] != 0 else 1.0
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="data/calibration/confidence_parameters_v2.json", help="Output JSON file path"
    )
