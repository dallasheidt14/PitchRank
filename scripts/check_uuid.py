...
try:
    home_games = (
        ...
        .select("game_uid, game_date, home_score, away_score")
        .eq("home_team_master_id", uuid_to_check)
        .limit(5)
        .execute()
    )
    if home_games.data:
        ...
        for game in home_games.data[:5]:
            print(
                f"      {game.get('game_date')}: {game.get('home_score')}-{game.get('away_score')} "
                f"({game.get('game_uid')})"
            )
    else:
...
try:
    away_games = (
        supabase.table("games")
        .select("game_uid, game_date, home_score, away_score")
        .eq("away_team_master_id", uuid_to_check)
        .limit(5)
        .execute()
    )
    if away_games.data:
