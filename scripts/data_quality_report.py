...
def get_quality_scores(cur, metrics):
    ...
    if scores["auto_merge_success_pct"] is not None:
        ...
        print("🔀 Auto-Merge Success Rate: N/A (no recent merges)")
...
def detect_anomalies(cur, metrics, scores):
            ...
            anomalies.append(
                {
                    "level": "CRITICAL",
                    "message": (
                        f"Quarantine rate spike: {quarantine_rate * 100:.1f}% of games in quarantine "
                        f"(>{THRESHOLDS['quarantine_rate_critical'] * 100:.0f}% threshold)"
                    ),
                }
            )
            critical_count += 1
            ...
            anomalies.append(
                {
                    "level": "WARNING",
                    "message": (
                        f"Potential duplicate rate: {dupe_rate * 100:.1f}% "
                        f"(>{THRESHOLDS['duplicate_rate_critical'] * 100:.0f}% threshold)"
                    ),
                }
            )
        ...
        anomalies.append(
            {
                "level": "WARNING",
                "message": (
                    f"{len(clubs_with_variants)} clubs have "
                    f">{THRESHOLDS['club_variants_critical']} name variants"
                ),
            }
        )
        for club_lower, variant_count in clubs_with_variants[:5]:
            anomalies.append({"level": "INFO", "message": f"   {club_lower}: {variant_count} variants"})
            ...
            anomalies.append(
                {
                    "level": "CRITICAL",
                    "message": (
                        f"Missing critical fields: {missing_rate * 100:.1f}% of teams "
                        f"(>{THRESHOLDS['missing_critical_fields'] * 100:.0f}% threshold)"
                    ),
                }
            )
            critical_count += 1
    ...
    if (
        metrics["review_queue_oldest_days"]
        and metrics["review_queue_oldest_days"] > THRESHOLDS["old_review_queue_days"]
    ):
        anomalies.append(
            {
                "level": "WARNING",
                "message": (
                    f"Review queue has entries {metrics['review_queue_oldest_days']} days old "
                    f"(>{THRESHOLDS['old_review_queue_days']} day threshold)"
                ),
            }
        )
    ...
    if metrics["games_added_7d"] == 0:
        anomalies.append({"level": "CRITICAL", "message": "Data pipeline down: 0 games added in last 7 days"})
        critical_count += 1
    ...
    if not anomalies:
        ...
        for anomaly in anomalies:
            level_icon = {"CRITICAL": "🔴", "WARNING": "⚠️", "INFO": "ℹ️"}.get(anomaly["level"], "•")
...
def compare_trends(metrics, scores):
    ...
    history_entry = {"timestamp": datetime.now().isoformat(), "metrics": metrics, "scores": scores}
