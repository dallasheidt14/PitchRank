...
from src.predictions.ml_match_predictor import MLMatchPredictor  # noqa: E402
...
def predict_match_hardcoded(team_a: Dict, team_b: Dict) -> Dict:
    ...
    if abs_power_diff < SKILL_GAP_THRESHOLDS["MEDIUM"]:
        ...
        weights = {
            ...
            "SOS": BASE_WEIGHTS["SOS"] + (BLOWOUT_WEIGHTS["SOS"] - BASE_WEIGHTS["SOS"]) * transition,
            "RECENT_FORM": BASE_WEIGHTS["RECENT_FORM"]
            + (BLOWOUT_WEIGHTS["RECENT_FORM"] - BASE_WEIGHTS["RECENT_FORM"]) * transition,
            "MATCHUP": BASE_WEIGHTS["MATCHUP"] + (BLOWOUT_WEIGHTS["MATCHUP"] - BASE_WEIGHTS["MATCHUP"]) * transition,
        }
...
async def build_team_histories_for_comparison(
    supabase: Client, team_ids: List[str], lookback_days: int = 365, max_teams: int = 5000, limit_per_team: int = 20
                ...
                for team_id in sub_batch:
                    ...
                    if not batch_games.empty:
                        ...
                        team_games = team_games.drop(
                            columns=["home_team_master_id_str", "away_team_master_id_str"], errors="ignore"
                        )
...
async def compare_predictors(
    games_df: pd.DataFrame, rankings_dict: Dict, ml_predictor: MLMatchPredictor, team_histories: Dict[str, pd.DataFrame]
) -> pd.DataFrame:
...
async def main():
    ...
    parser.add_argument("--limit", type=int, default=5000, help="Maximum number of games to test (default: 5000)")
    parser.add_argument(
        "--model-name", type=str, default="match_predictor", help="Name of saved ML model (default: match_predictor)"
    )
    ...
    try:
        ...
        print("ERROR: ML model not found. Train first with:")
        print("  python scripts/train_ml_match_predictor.py")
        sys.exit(1)
    ...
    try:
        ...
        team_ids = list(
            set(games_df["home_team_master_id"].dropna().unique())
