...
def get_profiles():
    ...
    response = requests.get(f"{BUFFER_API_URL}/profiles.json", params={"access_token": BUFFER_ACCESS_TOKEN})
...
def create_post(profile_id: str, text: str, media_url: str = None, scheduled_at: datetime = None):
    ...
    response = requests.post(f"{BUFFER_API_URL}/updates/create.json", data=data)
...
def post_movers_infographic(platform: str = "instagram", age_group: str = None, gender: str = None):
    ...
    result = create_post(profile_id=target_profile["id"], text=caption, media_url=infographic_url)
...
def main():
    ...
    parser.add_argument("--platform", type=str, default="instagram", help="Platform (instagram, twitter, facebook)")
    parser.add_argument("--age-group", type=str, help="Filter by age group")
    parser.add_argument("--gender", type=str, help="Filter by gender")
    parser.add_argument("--list-profiles", action="store_true", help="List connected Buffer profiles")
    ...
    success = post_movers_infographic(platform=args.platform, age_group=args.age_group, gender=args.gender)
