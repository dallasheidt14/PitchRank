...
# Sibling-script import idiom — needed to reach ``src.utils.X``.
...
# Single source of truth for the override registry (also consumed by the
# SOM Sports / future provider matchers' gated funnels).
from src.utils.club_canonical_overrides import (  # noqa: E402
    CLUB_CANONICAL_OVERRIDES,
)
from src.utils.club_canonical_overrides import (  # noqa: E402
    matches_override as _matches_override,
)
...
def fetch_no_state_teams(client):
    ...
    for is_null in (True, False):
        offset = 0
        while True:
            q = client.table("teams").select("team_id_master, team_name, club_name, gender").eq("is_deprecated", False)
...
def execute_fixes(client, all_fixes, dry_run=True):
    ...
    for fix in all_fixes:
        try:
            q = client.table("teams").update({"club_name": fix["to"]}).eq("club_name", fix["from"])
