...
def format_for_social(games):
    ...
    for game in sorted(games, key=lambda x: (x["date"], -(x["home_rank"] + game["away_rank"]))):
        ...
        print("\n💬 Social post:")
        print("🔥 MARQUEE MATCHUP 🔥")
        print(
            f"#{game['home_rank']} {game['home_info']['club_name']} "
            f"vs #{game['away_rank']} {game['away_info']['club_name']}"
        )
        print(f"📅 {date_str} | {game['event'][:30]}")
        print("#YouthSoccer #PitchRank")
