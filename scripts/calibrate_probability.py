...
def calculate_calibration_error(sensitivity: float, raw_df: pd.DataFrame, buckets: list) -> float:
    """
...
def find_optimal_sensitivity(raw_df: pd.DataFrame) -> dict:
    ...
    result = minimize_scalar(
        lambda s: calculate_calibration_error(s, raw_df, buckets), bounds=(2.0, 8.0), method="bounded"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="data/calibration/probability_parameters.json", help="Output JSON file path"
    )
