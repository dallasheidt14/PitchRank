...
def create_http_session() -> requests.Session:
    ...
    if CERTIFI_AVAILABLE:
        ...
        logger.debug("Using certifi certificates")
    ...
    if all_states:
        ...
        query = f"""
            WITH team_with_providers AS (
                ...
                {state_filter}
                AND LOWER(t.age_group) IN ('u10', 'u11', 'u12', 'u13', 'u14', 'u15', 'u16', 'u17', 'u18', 'u19')
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                logger.warning(
                    f"Timeout for team {team_id} (attempt {attempt + 1}/{MAX_RETRIES}), retrying in {wait_time:.1f}s..."
                )
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                logger.warning(
                    f"SSL error for team {team_id} (attempt {attempt + 1}/{MAX_RETRIES}), "
            ...
            if attempt < MAX_RETRIES - 1:
                wait_time = RETRY_DELAY * (attempt + 1)
                logger.warning(
                    f"Request error for team {team_id} (attempt {attempt + 1}/{MAX_RETRIES}): {e}, retrying..."
                )
                time.sleep(wait_time)
                continue
...
def main():
    ...
    parser.add_argument(
        "--all-states", action="store_true", help="Get top N teams per state/age/gender (use with --top-per-group)"
    )
    parser.add_argument("--states", type=str, help="Comma-separated state codes (e.g., CA,TX,AZ). Implies --all-states")
    parser.add_argument(
        "--top-per-group", type=int, default=25, help="Teams per group (default 25, used with --all-states or --states)"
    )
