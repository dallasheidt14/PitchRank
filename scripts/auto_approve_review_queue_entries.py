...
def main():
    ...
    entries_result = (
        ...
        .select("*")
        .eq("provider_id", MODULAR11_PROVIDER_CODE)
        .eq("status", "pending")
        .execute()
    )
    ...
    for entry in entries:
        ...
        aliased_provider_team_id = build_aliased_provider_team_id(provider_team_id, age_group, division)
        ...
        try:
            ...
            db.table("team_match_review_queue").update({"status": "approved"}).eq("id", entry["id"]).execute()
            approved_entries += 1
