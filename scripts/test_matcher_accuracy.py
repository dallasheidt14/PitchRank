...
from config.settings import SUPABASE_KEY, SUPABASE_URL  # noqa: E402
...
class MatcherAccuracyTester:
    ...
    def test_single_match(self, approved_entry: Dict) -> Dict:
        ...
        if not all([provider_team_name, age_group, gender]):
            return {"status": "skipped", "reason": "missing_required_fields", "entry": approved_entry}
    ...
    def print_summary(self):
        ...
        if total_valid > 0:
            ...
            confidence_buckets = {"0.90+": [], "0.85-0.89": [], "0.80-0.84": [], "0.75-0.79": [], "below 0.75": []}
            ...
            for bucket, results in confidence_buckets.items():
                if results:
                    ...
                    print(
                        f"  {bucket:12s}: {correct_in_bucket:4d}/{total_in_bucket:4d} "
                        f"correct ({accuracy_in_bucket:5.2f}%)"
                    )
    ...
    def save_results(self, output_file: str = None):
        ...
        summary = {
            ...
            if (self.total_tested - self.skipped) > 0
            else 0,
