...
def is_date_related_error(error_details: str) -> bool:
    ...
    date_keywords = ["date format", "invalid date", "unrecognized date", "date", "strptime"]
                ...
                try:
                    ...
                    if "timeout" in str(e).lower() or "57014" in str(e) or attempt == max_retries - 1:
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"Query timeout at offset {offset}, retrying (attempt {attempt + 1}/{max_retries})..."
                            )
                            import time
...
def display_summary(stats: Dict):
    ...
    if total > 0:
        ...
        table.add_row(
            "Date-Related Errors", f"{stats['date_related']:,}", f"{stats['date_related'] / total * 100:.1f}%"
        )
        table.add_row("Processed", f"{stats['processed']:,}", f"{stats['processed'] / total * 100:.1f}%")
        table.add_row(
            "Recovered",
            f"{stats['recovered']:,}",
            f"{stats['recovered'] / stats['processed'] * 100:.1f}%" if stats["processed"] > 0 else "0.0%",
        )
        table.add_row(
            "Duplicates (already in DB)",
            f"{stats['duplicates']:,}",
            f"{stats['duplicates'] / stats['processed'] * 100:.1f}%" if stats["processed"] > 0 else "0.0%",
        )
        table.add_row(
            "Failed",
            f"{stats['failed']:,}",
            f"{stats['failed'] / stats['processed'] * 100:.1f}%" if stats["processed"] > 0 else "0.0%",
        )
        table.add_row("Still Invalid", f"{stats['still_invalid']:,}", f"{stats['still_invalid'] / total * 100:.1f}%")
        table.add_row(
            "Deleted from Quarantine",
            f"{stats['deleted_from_quarantine']:,}",
            f"{stats['deleted_from_quarantine'] / stats['processed'] * 100:.1f}%" if stats["processed"] > 0 else "0.0%",
        )
    else:
...
async def main():
    parser = argparse.ArgumentParser(description="Reprocess quarantined games with fixed date formats")
    parser.add_argument("--provider", type=str, required=True, help='Provider code (e.g., "gotsport")')
    parser.add_argument(
        "--reason-code",
        type=str,
    ...
    parser.add_argument("--limit", type=int, default=None, help="Maximum number of games to process (default: all)")
    parser.add_argument("--batch-size", type=int, default=1000, help="Batch size for processing (default: 1000)")
    parser.add_argument("--dry-run", action="store_true", help="Dry run mode - do not insert or delete")
