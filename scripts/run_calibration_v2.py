...
def main():
    ...
    parser.add_argument("--wait", action="store_true", help="Wait for backtest to complete before running calibrations")
    parser.add_argument("--skip-copy", action="store_true", help="Skip copying JSONs to frontend")
    ...
    if args.wait:
        ...
        if not check_backtest_complete(csv_path):
            print("⚠️  Backtest CSV has fewer than 10,000 rows")
            print(f"   File: {csv_path}")
            if csv_path.exists():
                df = pd.read_csv(csv_path)
                print(f"   Current rows: {len(df):,}")
            print("\n   Run with --wait to wait for backtest completion")
            print("   Or run backtest manually:")
            print("   python scripts/backtest_predictor.py --lookback-days 365 --limit None --no-charts")
            return
