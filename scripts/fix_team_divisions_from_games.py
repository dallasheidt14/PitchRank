...
def analyze_team_divisions(db):
    ...
    games_result = (
        db.table("games")
        .select("id, home_team_master_id, away_team_master_id, competition, division_name")
    ...
    while games_result.data and len(games_result.data) > 0:
        ...
        try:
            games_result = (
                ...
                .select("id, home_team_master_id, away_team_master_id, competition, division_name")
                .eq("provider_id", MODULAR11_PROVIDER_ID)
                .range(len(all_games), len(all_games) + 1000)
                .execute()
            )
...
def rename_teams(db, teams_to_rename_hd, teams_to_rename_ad, dry_run=True):
    ...
    for team in teams_to_rename_hd:
        ...
        if dry_run:
            ...
            try:
                db.table("teams").update({"team_name": new_name}).eq("team_id_master", team["team_id"]).execute()
...
Examples:
  ...
  python scripts/fix_team_divisions_from_games.py --show-split-candidates
    ...
    if args.rename:
        rename_teams(db, analysis["rename_hd"], analysis["rename_ad"], dry_run=args.dry_run)
