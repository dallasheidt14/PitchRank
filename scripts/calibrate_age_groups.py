...
async def fetch_games_by_age_group(supabase: Client, lookback_days: int = 365) -> pd.DataFrame:
    """
    ...
    for i in range(0, len(team_ids), batch_size):
        ...
        try:
            response = (
                supabase.table("teams").select("team_id_master, age_group").in_("team_id_master", batch).execute()
            )
