...
incorrect_matches = [
    {"provider_team_name": "Barca Residency Academy U17", "matched_team_name": "Barca Residency Academy 09 EA AD"},
    {
...
aliases = (
    ...
    .select("id, provider_team_id, team_id_master, match_method, match_confidence, review_status")
    ...
    if team_result.data:
        ...
        for incorrect in incorrect_matches:
            if incorrect["matched_team_name"].lower() in team_name.lower():
                ...
                # Reject this alias
                try:
                    supabase.table("team_alias_map").update({"review_status": "rejected"}).eq("id", alias_id).execute()
