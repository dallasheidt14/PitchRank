...
async def main():
    ...
    if "sos" in teams_df.columns:
        ...
        top_teams = teams_df.nlargest(
            10, "powerscore_adj" if "powerscore_adj" in teams_df.columns else "powerscore_core"
        )
        ...
        for _, team in top_teams.head(10).iterrows():
            ...
            sample_table.add_row(
                ...
                f"{team.get('sos_norm', 'N/A'):.4f}"
                if "sos_norm" in teams_df.columns and pd.notna(team.get("sos_norm"))
                else "N/A",
                f"{team[power_col]:.4f}" if power_col in teams_df.columns else "N/A",
            )
        console.print(sample_table)
    else:
        console.print("  [red]✗ SOS column not found in rankings output[/red]")
    ...
    try:
        ...
        rankings_result = (
            ...
            .select("team_id, strength_of_schedule, national_power_score, games_played")
            .limit(100)
            .execute()
        )
        ...
        if rankings_result.data:
            ...
            if sos_in_db > 0:
                ...
                console.print("  [red]✗ SOS is NOT stored in database (all values are NULL)[/red]")
                console.print("  [yellow]This means the save function is not including SOS![/yellow]")
        else:
            console.print("  [yellow]No rankings found in database[/yellow]")
