...
def get_mls_next_teams(db):
    ...
    aliases_result = (
        db.table("team_alias_map").select("team_id_master").eq("provider_id", MODULAR11_PROVIDER_ID).execute()
    )
...
def get_base_club_id(db, team_id_master: str) -> str:
    ...
    games_result = (
        db.table("games")
        .select("home_team_master_id, away_team_master_id, home_provider_id, away_provider_id")
        .eq("provider_id", MODULAR11_PROVIDER_ID)
        .or_(f"home_team_master_id.eq.{team_id_master},away_team_master_id.eq.{team_id_master}")
        .limit(100)
        .execute()
    )
...
def determine_division_from_games(db, team_id_master: str, team_name: str = None) -> str:
    ...
    games_result = (
        db.table("games")
        .select("competition, division_name")
        .eq("provider_id", MODULAR11_PROVIDER_ID)
        .or_(f"home_team_master_id.eq.{team_id_master},away_team_master_id.eq.{team_id_master}")
        .limit(1000)
        .execute()
...
def create_proper_alias(
    db, team_id_master: str, base_club_id: str, age_group: str, division: str = None, dry_run: bool = True
):
    """
    ...
    alias_exists = (
        ...
        .eq("provider_id", MODULAR11_PROVIDER_ID)
        .eq("provider_team_id", new_alias_id)
        .execute()
    )
  ...
  python scripts/migrate_modular11_divisions_from_games.py --verbose
    ...
    for i, team in enumerate(teams, 1):
        ...
        if not division:
            stats["no_division"] += 1
            if args.verbose:
                print("  ⚠️  No division found (will create alias without division)")
        ...
        if success:
            ...
            if "already exists" in message.lower() or "already has" in message.lower():
                ...
                stats["conflicts"] += 1
                teams_needing_review.append(
                    {"team_id": team_id, "team_name": team_name, "age_group": age_group, "reason": message}
                )
                if args.verbose:
                    print(f"  ⚠️  {message}")
            else:
    ...
    if teams_needing_review:
        ...
        if args.verbose and not args.dry_run:
            csv_path = (
                ...
                / "data"
                / "exports"
                / f"teams_needing_review_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
