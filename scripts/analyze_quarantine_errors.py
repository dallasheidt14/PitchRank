...
def analyze_quarantine_errors(limit: int = None, reason_code: str = None):
    ...
    for category, count in error_categories.most_common():
        ...
        error_table.add_row(
            category, f"{count:,}", f"{percentage:.1f}%", sample[:80] + "..." if len(sample) > 80 else sample
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze validation errors in quarantined games")
    parser.add_argument("--reason-code", type=str, default=None, help="Filter by reason code (default: all)")
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of games to analyze (default: 10,000 sample)"
    )
