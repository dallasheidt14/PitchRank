...
class AliasReviewer:
    ...
    def review_pending(self):
        ...
        while True:
            ...
            table.add_row(
                "Confidence",
                f"[{'green' if alias['match_confidence'] > 0.8 else 'yellow'}]{alias['match_confidence']:.2f}[/]",
            )
    ...
    def _approve_alias(self, alias_id: str):
        ...
        self.supabase.table("team_alias_map").update(
            {"review_status": "approved", "reviewed_by": "admin", "reviewed_at": datetime.now().isoformat()}
        ).eq("id", alias_id).execute()

    def _reject_alias(self, alias_id: str):
        """Reject an alias"""
        self.supabase.table("team_alias_map").update(
            {"review_status": "rejected", "reviewed_by": "admin", "reviewed_at": datetime.now().isoformat()}
        ).eq("id", alias_id).execute()

    def _mark_new_team(self, alias_id: str):
        """Mark as new team needing addition to master list"""
        self.supabase.table("team_alias_map").update(
            {"review_status": "new_team", "reviewed_by": "admin", "reviewed_at": datetime.now().isoformat()}
        ).eq("id", alias_id).execute()
