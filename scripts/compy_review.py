...
def get_recent_sessions(hours: int = 24):
    ...
    for session_file in sessions_dir.glob("*.jsonl"):
        ...
        if mtime > cutoff:
            recent.append({"path": session_file, "modified": mtime, "size": session_file.stat().st_size})
...
def main():
    ...
    parser.add_argument("--output", choices=["json", "summary", "compy"], default="compy", help="Output format")
    parser.add_argument(
        "--preflight",
        action="store_true",
    ...
    if args.preflight:
        ...
        for session in sessions:
            parsed = parse_session(session["path"])
            # Consider meaningful if >5 messages or has errors
            if parsed.get("message_count", 0) > 5 or parsed.get("errors"):
