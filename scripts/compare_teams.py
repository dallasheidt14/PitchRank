...
print("Total records in CSV: 266 (133 games × 2 perspectives)")

# Get teams that were created during import
created_teams_result = (
    supabase.table("teams")
    .select("provider_team_id, team_name, created_at")
    .eq("provider_id", tgs_provider_id)
    .gte("created_at", "2025-12-11T20:29:00")
    .lte("created_at", "2025-12-11T20:31:30")
    .execute()
)
...
if existing_team_ids:
    ...
    existing_teams_result = (
        supabase.table("teams")
        .select("provider_team_id, team_name, created_at")
        .eq("provider_id", tgs_provider_id)
        .in_("provider_team_id", list(existing_team_ids)[:10])
        .execute()
    )
