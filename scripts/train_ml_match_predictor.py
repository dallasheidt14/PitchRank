...
async def build_team_histories(
    supabase: Client, games_df: pd.DataFrame, lookback_days: int = 365, max_teams: int = 5000, limit_per_team: int = 20
...
async def main():
    ...
    parser.add_argument("--min-games", type=int, default=5, help="Minimum games per team to include (default: 5)")
    parser.add_argument(
        "--model-name", type=str, default="match_predictor", help="Name for saved model (default: match_predictor)"
    )
    parser.add_argument("--test-size", type=float, default=0.2, help="Proportion of data for testing (default: 0.2)")
    parser.add_argument(
        "--limit", type=int, default=20000, help="Maximum number of games to fetch for training (default: 20000)"
    )
    parser.add_argument(
        "--use-full-rankings", action="store_true", help="Use rankings_full table instead of rankings_view"
    )
    ...
    predictor = MLMatchPredictor(min_games_per_team=args.min_games)
