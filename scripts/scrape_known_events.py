...
def scrape_known_events(event_ids: List[str], lookback_days: int = 30, output_file: str = None):
    """
            ...
            try:
                ...
                if not team_ids:
                    event_results.append(
                        {"event_id": event_id, "teams_count": 0, "games_count": 0, "status": "no_teams"}
                    )
                ...
                event_results.append(
                    {"event_id": event_id, "teams_count": len(team_ids), "games_count": len(games), "status": "success"}
                )
                ...
                event_results.append(
                    {"event_id": event_id, "teams_count": 0, "games_count": 0, "status": "error", "error": str(e)}
                )
    ...
    for result in event_results:
        status_icon = {"success": "✅", "no_teams": "⚠️", "error": "❌"}.get(result["status"], "?")

        summary_table.add_row(result["event_id"], str(result["teams_count"]), str(result["games_count"]), status_icon)
...
Examples:
  ...
  python scripts/scrape_known_events.py --event-ids 40550 --lookback-days 60
    ...
    group.add_argument("--event-ids", type=str, help='Comma-separated list of event IDs (e.g., "40550,40551")')
    group.add_argument("--events-file", type=str, help="JSON file with list of event IDs")
    ...
    scrape_known_events(event_ids=event_ids, lookback_days=args.lookback_days, output_file=args.output)
