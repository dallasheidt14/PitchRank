...
def check_database_readiness():
    ...
    checks = {"indexes": True, "tables": True, "triggers": True, "permissions": True}
    ...
    try:
        ...
        supabase.table("games").select("game_uid,is_immutable,original_import_id").limit(1).execute()
    ...
    try:
        supabase.table("build_logs").select("metrics").limit(1).execute()
        console.print("  [green]✓[/green] build_logs.metrics (JSONB)")
    except Exception as e:
